...
//        appear!
// - 5. Call EndTable()
IMGUI_API bool BeginTable(const char *str_id, int columns,
...
// Obsolete names
  ...
  ImGuiWindowFlags_NavFlattened =
      1 << 31, // Obsoleted in 1.90.9: Use ImGuiChildFlags_NavFlattened in
               // BeginChild() call.
#endif
};
...
enum ImGuiChildFlags_ {
  ...
  ImGuiChildFlags_NavFlattened =
      1 << 8, // Share focus scope, allow gamepad/keyboard navigation to cross
              // over parent border to this child or between sibling child
              // windows.
};
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_ParseEmptyRefVal =
      1 << 13, // InputFloat(), InputInt(), InputScalar() etc. only: parse empty
               // string as zero value.
  ImGuiInputTextFlags_DisplayEmptyRefVal =
      1 << 14, // InputFloat(), InputInt(), InputScalar() etc. only: when value
               // is zero, do not display it. Generally used with
               // ImGuiInputTextFlags_ParseEmptyRefVal.
  ...
  ImGuiInputTextFlags_CallbackCompletion =
      1 << 17, // Callback on pressing TAB (for completion handling)
  ImGuiInputTextFlags_CallbackHistory =
      1 << 18, // Callback on pressing Up/Down arrows (for history handling)
  ImGuiInputTextFlags_CallbackAlways =
      1 << 19, // Callback on each iteration. User code may query cursor
               // position, modify text buffer.
  ImGuiInputTextFlags_CallbackCharFilter =
      1 << 20, // Callback on character inputs to replace or discard them.
               // Modify 'EventChar' to replace or discard, or return 1 in
               // callback to discard.
  ImGuiInputTextFlags_CallbackResize =
      1 << 21, // Callback on buffer capacity changes request (beyond 'buf_size'
  ...
  ImGuiInputTextFlags_CallbackEdit =
      1 << 22, // Callback on any edit (note that InputText() already returns
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_FramePadding =
      1
      << 10, // Use FramePadding (even for an unframed text node) to vertically
  ...
  ImGuiTreeNodeFlags_SpanAllColumns =
      1 << 14, // Frame will span all columns of its container table (text will
               // still fit in current column)
  ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
      1 << 15, // (WIP) Nav: left direction may move to this TreeNode() from any
...
enum ImGuiInputFlags_ {
  ...
  // (if owner is active item) >> RouteGlobal+OverFocused >> RouteFocused (if in
  // focused window stack) >> RouteGlobal.
  ...
  ImGuiInputFlags_RouteGlobal = 1
                                << 12, // Global route (unless a focused window
                                       // or active item registered the route).
  ...
  ImGuiInputFlags_RouteOverActive =
      1
      << 15, // Option: global route: higher priority than active item. Unlikely
             // you need to use that: will interfere with every active items,
             // e.g. CTRL+A registered by InputText will be overridden by this.
             // May not be fully honored as user/internal code is likely to
             // always assume they can access keys when active.
  ...
  ImGuiInputFlags_RouteFromRootWindow =
      1 << 17, // Option: route evaluated from the point of view of root window
...
struct ImGuiStyle {
  ...
  ImVec2 CellPadding; // Padding within a table cell. Cellpadding.x is locked
                      // for entire table. CellPadding.y may be altered between
                      // different rows.
  ...
  ImVec2 DisplayWindowPadding; // Apply to regular windows: amount which we
                               // enforce to keep visible when moving near edges
                               // of your screen.
  ImVec2 DisplaySafeAreaPadding; // Apply to every windows, menus, popups,
                                 // tooltips: amount where we avoid displaying
                                 // contents. Adjust if you cannot see the edges
                                 // of your screen (e.g. on a TV where scaling
...
struct ImGuiIO {
  ...
  bool ConfigMacOSXBehaviors; // = defined(__APPLE__) // Swap Cmd<>Ctrl keys +
                              ...
                              // Alt instead of Ctrl, Shortcuts using Cmd/Super
                              // instead of Ctrl, Line/Text Start and End using
  ...
  IMGUI_API void
  ClearInputKeys(); // Clear current keyboard/gamepad state + current frame text
  ...
  ImGuiKeyChord
      KeyMods; // Key mods flags (any of
               ...
               // into flags. Read-only, updated by NewFrame()
  ImGuiKeyData
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  // typedef ::ImGuiStoragePair ImGuiStoragePair;  // 1.90.8: moved type outside
  // struct
#endif
};
...
struct ImGuiViewport {
  ...
  void *PlatformHandle;    // void* to hold higher-level, platform window handle
                           // (e.g. HWND, GLFWWindow*, SDL_Window*)
...
IMGUI_API ImGuiKey GetKeyIndex(
    ImGuiKey key); // Map ImGuiKey_* values into legacy native key index. ==
                   // io.KeyMap[key]. When using a 1.87+ backend using
                   // io.AddKeyEvent(), calling GetKeyIndex() with ANY
