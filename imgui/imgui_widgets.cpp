    ...
    if (!(flags & ImGuiButtonFlags_NoKeyModifiers) ||
      ...
      if (g.ActiveId == id && (item_flags & ImGuiItemFlags_ButtonRepeat))
        if (g.IO.MouseDownDuration[g.ActiveIdMouseButton] > 0.0f &&
            IsMouseClicked(g.ActiveIdMouseButton, ImGuiInputFlags_Repeat,
                           test_owner_id))
          pressed = true;
    }
...
void ImGui::Scrollbar(ImGuiAxis axis) {
  ...
  ScrollbarEx(bb, id, axis, &scroll, (ImS64)size_visible, (ImS64)size_contents,
              rounding_corners);
    ...
    if (g.ScrollbarSeekMode == 0) {
      ...
      *p_scroll_v = (ImS64)(scroll_v_norm * scroll_max);
      ...
      if (IsMouseClicked(ImGuiMouseButton_Left, ImGuiInputFlags_Repeat) &&
        ...
        *p_scroll_v = ImClamp(*p_scroll_v + (ImS64)(page_dir * size_visible_v),
                              (ImS64)0, scroll_max);
      }
    }
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  if (g.ActiveId == id) {
    ...
    // FIXME: May be a problem to always steal Alt on OSX, would ideally still
    // allow an uninterrupted Alt down-up to toggle menu
    if (is_osx)
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    const bool is_paste =
        ...
        !is_readonly;
    const bool is_undo = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Z, f_repeat, id)) &&
        ...
        if (event_flag) {
          ...
          if (buf_dirty) {
            ...
            InputTextReconcileUndoStateAfterUserCallback(
                ...
                callback_data.BufTextLen); // FIXME: Move the rest of this block
