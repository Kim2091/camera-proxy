...
static inline ImVec2 ImClamp(const ImVec2 &v, const ImVec2 &mn,
                             const ImVec2 &mx) {
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_HasClipRect = 1
                                     << 9, // g.LastItemData.ClipRect is valid.
...
struct ImGuiLastItemData {
  ...
  // corresponding ImGuiItemStatusFlags ar set.
  ImRect DisplayRect; // Display rectangle. ONLY VALID IF (StatusFlags &
                      // ImGuiItemStatusFlags_HasDisplayRect) is set.
  ImRect
      ClipRect; // Clip rectangle at the time of submitting item. ONLY VALID IF
                // (StatusFlags & ImGuiItemStatusFlags_HasClipRect) is set..
  ImGuiKeyChord
...
struct ImGuiNavItemData {
  ...
  ImGuiSelectionUserData
      SelectionUserData; // I+Mov    // Best candidate
                         ...
                         // (InFlags & ImGuiItemFlags_HasSelectionUserData)
...
struct ImGuiContext {
  ...
  char ContextName[16]; // Storage for a context name (to facilitate debugging
  ...
  bool HoveredIdIsDisabled; // At least one widget passed the rect test, but has
  ...
  ImGuiNavLayer
      NavLayer; // Focused layer (main scrolling layer, or menu/title bar layer)
  ImGuiID
  ...
  bool NavJustMovedToIsTabbing; // Copy of ImGuiNavMoveFlags_IsTabbing. Maybe we
                                // should store whole flags.
  bool NavJustMovedToHasSelectionData; // Copy of move result's InFlags &
                                       // ImGuiItemFlags_HasSelectionUserData).
  ...
  ImGuiKeyChord
      ConfigNavWindowingKeyNext; // = ImGuiMod_Ctrl | ImGuiKey_Tab (or
                                 // ImGuiMod_Super | ImGuiKey_Tab on OS X). For
                                 // reconfiguration (see #4828)
  ImGuiKeyChord
  ...
  ImGuiContext(ImFontAtlas *shared_font_atlas) {
    ...
    // FIXME: Because this value is stored, it annoyingly interfere with
    // toggling io.ConfigMacOSXBehaviors updating this..
    ConfigNavWindowingKeyNext = IO.ConfigMacOSXBehaviors
                                    ? (ImGuiMod_Super | ImGuiKey_Tab)
...
struct IMGUI_API ImGuiWindow {
  ...
  ImGuiCond SetWindowPosAllowFlags : 8;  // store acceptable condition flags for
                                         // SetNextWindowPos() use.
  ImGuiCond SetWindowSizeAllowFlags : 8; // store acceptable condition flags for
                                         // SetNextWindowSize() use.
  ImGuiCond
      SetWindowCollapsedAllowFlags : 8; // store acceptable condition flags for
...
struct ImGuiTableColumn {
  ...
  ImU8 SortDirection : 2;    // ImGuiSortDirection_Ascending or
                             // ImGuiSortDirection_Descending
  ImU8 SortDirectionsAvailCount : 2; // Number of available sort directions (0
                                     // to 3)
  ImU8 SortDirectionsAvailMask : 4;  // Mask of available sort directions (1-bit
                                     // each)
...
IMGUI_API void
FindHoveredWindowEx(const ImVec2 &pos, bool find_first_and_in_any_viewport,
...
IMGUI_API bool IsMouseClicked(ImGuiMouseButton button, ImGuiInputFlags flags,
                              ImGuiID owner_id = 0);
...
IMGUI_API bool Shortcut(ImGuiKeyChord key_chord, ImGuiInputFlags flags,
                        ImGuiID owner_id);
