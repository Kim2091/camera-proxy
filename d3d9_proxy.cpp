...
extern IMGUI_IMPL_API auto ImGui_ImplWin32_WndProcHandler(HWND hWnd,
                                                             UINT msg,
                                                             WPARAM wParam,
                                                             LPARAM lParam) -> LRESULT;

auto LooksLikeMatrix(const float *data) -> bool;
static void LogMsg(const char *fmt, ...);
static auto LooksLikeViewStrict(const D3DMATRIX &m) -> bool;
static auto LooksLikeProjectionStrict(const D3DMATRIX &m) -> bool;
auto ExtractFOV(const D3DMATRIX &proj) -> float;

static auto Dot3(float ax, float ay, float az, float bx, float by, float bz) -> float;
static void CopyFilenameFromPath(const char *fullPath, char *outName,
                                 size_t outNameSize);
static auto TryReadFileDescription(const char *exePath, char *outDescription,
                                   size_t outDescriptionSize) -> bool;
...
static auto AnalyzeProjectionMatrixNumeric(const D3DMATRIX &m,
                                           ProjectionAnalysis *out) -> bool;
static auto ProjectionHandednessLabel(ProjectionHandedness handedness) -> const char *;
static auto InvertMatrix4x4Deterministic(const D3DMATRIX &in, D3DMATRIX *out,
                                         float *outDeterminant = nullptr) -> bool;
...
static const char *kCameraProxyVersion =
    "Camera Proxy v0.2.0 experimental branch";
...
struct ProxyConfig {
  ...
  CustomProjectionMode experimentalCustomProjectionMode =
      CustomProjectionMode_Auto;
  ...
  ProjectionHandedness experimentalCustomProjectionAutoHandedness =
      ProjectionHandedness_Left;
...
static bool g_profileCoreRegistersSeen[3] = {false, false,
                                             false}; // proj, viewInv, world
...
static void UpdateMatrixSource(MatrixSlot slot, uintptr_t shaderKey,
                               int baseRegister, int rows, bool transposed,
                               bool manual, const char *sourceLabel = nullptr,
                               int extractedFromRegister = -1);
...
static ProjectionHandedness g_projectionDetectedHandedness =
    ProjectionHandedness_Unknown;
...
static bool g_gameSetTransformSeen[3] = {false, false,
                                         false}; // world, view, projection
...
enum ShaderStageType { ShaderStage_Vertex = 0, ShaderStage_Pixel = 1 };
...
static auto TrimCopy(const std::string &s) -> std::string {
  size_t b = 0;
...
static auto ParseWriteMaskBits(const std::string &reg) -> int {
  size_t dot = reg.find('.');
  if (dot == std::string::npos || dot + 1 >= reg.size()) {
    return 0xF;
}
  int mask = 0;
...
static auto ExtractConstRegister(const std::string &operand, int *outReg) -> bool {
  if (outReg == nullptr) {
    return false;
}
  size_t cpos = operand.find('c');
  if (cpos == std::string::npos || cpos + 1 >= operand.size()) {
    return false;
}
  if (std::isdigit(static_cast<unsigned char>(operand[cpos + 1])) == 0) {
    return false;
}
  int v = 0;
  size_t i = cpos + 1;
  while (i < operand.size() &&
         (std::isdigit(static_cast<unsigned char>(operand[i])) != 0)) {
    v = (v * 10) + (operand[i] - '0');
    ++i;
  }
  *outReg = v;
...
static auto SplitAsmOperands(const std::string &text) -> std::vector<std::string> {
...
static auto IsOpcodeModifierChar(char c) -> bool {
  return c == '_' || c == 'x' || c == 'y' || c == 'z' || c == 'w' || c == 'p' ||
         c == 's' || c == 't';
}

static auto NormalizeOpcodeRoot(const std::string &opcode) -> std::string {
  size_t p = opcode.find('_');
  if (p == std::string::npos) {
    return opcode;
}
  if (p + 1 < opcode.size() &&
      (std::isdigit(static_cast<unsigned char>(opcode[p + 1])) != 0)) {
    return opcode;
  }
  size_t i = p + 1;
  while (i < opcode.size() && IsOpcodeModifierChar(opcode[i])) {
    ++i;
}
  if (i < opcode.size() && opcode[i] == '_') {
    return opcode.substr(0, i);
}
  return opcode.substr(0, p);
}

static auto ParseAsmInstructionLine(const std::string &line) -> ParsedAsmInstruction {
  ...
  if ((t.rfind("vs_", 0) == 0 || t.rfind("ps_", 0) == 0) &&
      t.find(' ') == std::string::npos) {
    return parsed;
  }
  size_t sp = t.find(' ');
  parsed.opcode = NormalizeOpcodeRoot(
      TrimCopy(sp == std::string::npos ? t : t.substr(0, sp)));
...
static auto LoadD3DCompilerModule() -> HMODULE {
...
static auto BuildShaderReplacementFromAssembly(IDirect3DDevice9 *device,
                                               ShaderRecord *rec,
                                               std::string *outError) -> bool {
  if ((device == nullptr) || (rec == nullptr)) {
    if (outError != nullptr) {
      *outError = "Invalid device or shader record.";
}
    return false;
  }
  if (rec->editableAssembly.empty()) {
    if (outError != nullptr) {
      *outError = "Assembly editor is empty.";
}
    return false;
  }
  ...
  if (compiler == nullptr) {
    if (outError != nullptr) {
      *outError = "d3dcompiler_47.dll / d3dcompiler_43.dll not available.";
}
    return false;
  }

  typedef HRESULT(WINAPI * D3DAssembleFn)(
      LPCVOID, SIZE_T, LPCSTR, const D3D_SHADER_MACRO *, ID3DInclude *, UINT,
      ID3DBlob **, ID3DBlob **);
  auto assembleFn =
      reinterpret_cast<D3DAssembleFn>(GetProcAddress(compiler, "D3DAssemble"));
  if (assembleFn == nullptr) {
    if (outError != nullptr) {
      *outError = "D3DAssemble is unavailable in loaded d3dcompiler.";
}
    return false;
  }
  ...
  HRESULT asmHr =
      assembleFn(rec->editableAssembly.c_str(), rec->editableAssembly.size(),
                 nullptr, nullptr, nullptr, 0, &assembled, &errors);
  if (FAILED(asmHr) || (assembled == nullptr)) {
    if (outError != nullptr) {
      if ((errors != nullptr) && (errors->GetBufferPointer() != nullptr)) {
        *outError = static_cast<const char *>(errors->GetBufferPointer());
      } else {
        char msg[128] = {};
        snprintf(msg, sizeof(msg), "Assembly failed: HRESULT=0x%08X",
                 static_cast<unsigned int>(asmHr));
        *outError = msg;
      }
    }
    if (errors != nullptr) {
  ...
  if (rec->stage == ShaderStage_Vertex) {
    ...
    createHr = device->CreateVertexShader(
        reinterpret_cast<const DWORD *>(assembled->GetBufferPointer()),
        &shader);
    replacement = shader;
  } else {
  ...
  if (FAILED(createHr) || (replacement == nullptr)) {
    if (outError != nullptr) {
      char msg[128] = {};
      snprintf(msg, sizeof(msg),
               "Device shader creation failed: HRESULT=0x%08X",
  ...
  if (rec->replacementShader != nullptr) {
  ...
  rec->modifiedBytecode.assign(
      reinterpret_cast<const uint32_t *>(assembled->GetBufferPointer()),
      reinterpret_cast<const uint32_t *>(assembled->GetBufferPointer()) +
          dwordCount);
...
static auto BuildD3DDisassembly(const std::vector<uint32_t> &bytecode) -> std::string {
  if (bytecode.empty()) {
    return "";
}
  HMODULE compiler = LoadD3DCompilerModule();
  if (compiler == nullptr) {
    return "<d3dcompiler not available>";
}
  typedef HRESULT(WINAPI * D3DDisassembleFn)(LPCVOID, SIZE_T, UINT, LPCSTR,
                                             ID3DBlob **);
  auto fn = reinterpret_cast<D3DDisassembleFn>(
      GetProcAddress(compiler, "D3DDisassemble"));
  if (fn == nullptr) {
    return "<D3DDisassemble missing>";
}
  ID3DBlob *blob = nullptr;
  HRESULT hr = fn(bytecode.data(), bytecode.size() * sizeof(uint32_t), 0,
                  nullptr, &blob);
...
static auto
BuildIrFromDisassembly(const std::string &disasm) -> std::vector<ShaderIrInstruction> {
  std::vector<ShaderIrInstruction> ir;
  ...
  while (std::getline(in, line)) {
    ...
    std::string rest =
        sp == std::string::npos ? "" : TrimCopy(t.substr(sp + 1));
    if (!rest.empty()) {
      std::vector<std::string> ops;
      size_t start = 0;
      while (start < rest.size()) {
        size_t comma = rest.find(',', start);
        std::string token = TrimCopy(
...
static void ClassifyShaderRecord(ShaderRecord *rec) {
  ...
  for (size_t i = 0; i < rec->ir.size(); ++i) {
    ...
    if (inst.opcode == "if" || inst.opcode == "loop" || inst.opcode == "rep" ||
        inst.opcode == "call" || inst.opcode == "callnz") {
      rec->usesFlowControl = true;
    }
    if (inst.dst.find("a0") != std::string::npos ||
        (inst.dst.find('[') != std::string::npos &&
         inst.dst.find('c') != std::string::npos)) {
      rec->usesSkinning = true;
    }
    for (const std::string &src : inst.src) {
      int c = -1;
      if (ExtractConstRegister(src, &c) && c >= 0 &&
          c < kMaxConstantRegisters) {
        rec->constantUsage[c] = true;
        if (c < minConstReg) {
          minConstReg = c;
}
      }
      if (src.find("a0") != std::string::npos ||
          src.find("[a0") != std::string::npos) {
        rec->usesSkinning = true;
}
    }
  }
  ...
  for (size_t i = 0; i + 3 < rec->ir.size(); ++i) {
    ...
    if (a.opcode == "dp4" && b.opcode == "dp4" && c.opcode == "dp4" &&
        d.opcode == "dp4" && a.dst.find("oPos") != std::string::npos &&
        b.dst.find("oPos") != std::string::npos &&
        c.dst.find("oPos") != std::string::npos &&
        d.dst.find("oPos") != std::string::npos && a.src.size() >= 2 &&
        b.src.size() >= 2 && c.src.size() >= 2 && d.src.size() >= 2 &&
        a.src[0] == b.src[0] && b.src[0] == c.src[0] && c.src[0] == d.src[0]) {
      int c0 = -1;
      int c1 = -1;
      int c2 = -1;
      int c3 = -1;
      if (ExtractConstRegister(a.src[1], &c0) &&
          ExtractConstRegister(b.src[1], &c1) &&
          ExtractConstRegister(c.src[1], &c2) &&
          ExtractConstRegister(d.src[1], &c3) && c1 == c0 + 1 && c2 == c0 + 2 &&
          c3 == c0 + 3) {
  ...
  bool sawDot = false;
  bool sawMaxZero = false;
  bool sawMul = false;
  for (const ShaderIrInstruction &inst : rec->ir) {
    sawDot |= (inst.opcode == "dp3" || inst.opcode == "dp4");
    if (inst.opcode == "max" && inst.src.size() >= 2 &&
        (inst.src[1] == "c0.x" || inst.src[1] == "0")) {
      sawMaxZero = true;
}
...
static auto TryGetShaderBytecodeHash(uintptr_t shaderKey, uint32_t *outHash) -> bool {
  if ((outHash == nullptr) || shaderKey == 0) {
    return false;
  }
...
static auto GetShaderState(uintptr_t shaderKey,
                                           bool createIfMissing) -> ShaderConstantState *;
...
static LRESULT CALLBACK ImGuiWndProcHook(HWND hwnd, UINT msg, WPARAM wParam,
                                         LPARAM lParam) {
  if (g_imguiInitialized) {
    ...
    if (g_showImGui) {
      ...
      const bool keyboardMsg = (msg >= WM_KEYFIRST && msg <= WM_KEYLAST) ||
  ...
  if (g_imguiPrevWndProc != nullptr) {
    return CallWindowProc(g_imguiPrevWndProc, hwnd, msg, wParam, lParam);
  }
  return DefWindowProc(hwnd, msg, wParam, lParam);
}

static auto ShouldBypassInputForImGuiMenu() -> bool {
  return g_imguiInitialized && g_showImGui &&
         g_config.disableGameInputWhileMenuOpen;
}

static auto IsProxyHotkeyVk(DWORD vkCode) -> bool {
...
static LRESULT CALLBACK LowLevelKeyboardBlockHook(int nCode, WPARAM wParam,
                                                  LPARAM lParam) {
  if (nCode == HC_ACTION && ShouldBypassInputForImGuiMenu()) {
    const auto *keyInfo =
        reinterpret_cast<const KBDLLHOOKSTRUCT *>(lParam);
    if ((keyInfo != nullptr) && !IsProxyHotkeyVk(keyInfo->vkCode)) {
      return 1;
    }
  }
...
static LRESULT CALLBACK LowLevelMouseBlockHook(int nCode, WPARAM wParam,
                                               LPARAM lParam) {
  if (nCode == HC_ACTION && ShouldBypassInputForImGuiMenu()) {
    return 1;
  }
...
static void UpdateInputBlockHooks() {
  ...
  if (shouldBlock) {
    if (g_keyboardBlockHook == nullptr) {
      g_keyboardBlockHook = SetWindowsHookExA(
          WH_KEYBOARD_LL, LowLevelKeyboardBlockHook, g_moduleInstance, 0);
    }
    if (g_mouseBlockHook == nullptr) {
      g_mouseBlockHook = SetWindowsHookExA(WH_MOUSE_LL, LowLevelMouseBlockHook,
                                           g_moduleInstance, 0);
    }
  } else {
    if (g_keyboardBlockHook != nullptr) {
...
static auto VkToImGuiKey(UINT vk) -> ImGuiKey {
  if (vk >= '0' && vk <= '9') {
  ...
  for (UINT vk = 0; vk < 256; ++vk) {
    ...
    if (key != ImGuiKey_None) {
      ...
      io.SetKeyEventNativeData(
          key, vk, static_cast<int>(MapVirtualKeyA(vk, MAPVK_VK_TO_VSC)));
    }
    if (down) {
      ...
      if (GetKeyboardState(keyboardState) != 0) {
        ...
        const int translated =
            ToUnicode(vk, MapVirtualKeyA(vk, MAPVK_VK_TO_VSC), keyboardState,
                      utf16Buf, 4, 0);
        if (translated > 0) {
...
static void EnsureWndProcHookInstalled() {
  if (!g_imguiInitialized || (g_imguiHwnd == nullptr)) {
    return;
  }
  auto current =
      reinterpret_cast<WNDPROC>(GetWindowLongPtr(g_imguiHwnd, GWLP_WNDPROC));
...
static void StoreViewMatrix(const D3DMATRIX &view, uintptr_t shaderKey = 0,
                            int baseRegister = -1, int rows = 4,
                            bool transposed = false, bool manual = false,
  ...
  UpdateMatrixSource(MatrixSlot_View, shaderKey, baseRegister, rows, transposed,
                     manual, sourceLabel, extractedFromRegister);
}
...
static void StoreProjectionMatrix(const D3DMATRIX &projection,
  ...
  UpdateMatrixSource(MatrixSlot_Projection, shaderKey, baseRegister, rows,
                     transposed, manual, sourceLabel, extractedFromRegister);
}

static void StoreWorldMatrix(const D3DMATRIX &world, uintptr_t shaderKey = 0,
                             int baseRegister = -1, int rows = 4,
                             bool transposed = false, bool manual = false,
                             const char *sourceLabel = nullptr,
                             int extractedFromRegister = -1) {
  ...
  UpdateMatrixSource(MatrixSlot_World, shaderKey, baseRegister, rows,
                     transposed, manual, sourceLabel, extractedFromRegister);
}

static void StoreMVPMatrix(const D3DMATRIX &mvp, uintptr_t shaderKey = 0,
                           int baseRegister = -1, int rows = 4,
                           bool transposed = false, bool manual = false,
                           const char *sourceLabel = nullptr,
                           int extractedFromRegister = -1) {
  ...
  UpdateMatrixSource(MatrixSlot_MVP, shaderKey, baseRegister, rows, transposed,
                     manual, sourceLabel, extractedFromRegister);
}

static void StoreVPMatrix(const D3DMATRIX &vp, uintptr_t shaderKey = 0,
                          int baseRegister = -1, int rows = 4,
                          bool transposed = false, bool manual = false,
                          const char *sourceLabel = nullptr,
                          int extractedFromRegister = -1) {
  ...
  UpdateMatrixSource(MatrixSlot_VP, shaderKey, baseRegister, rows, transposed,
                     manual, sourceLabel, extractedFromRegister);
}

static void StoreWVMatrix(const D3DMATRIX &wv, uintptr_t shaderKey = 0,
                          int baseRegister = -1, int rows = 4,
                          bool transposed = false, bool manual = false,
                          const char *sourceLabel = nullptr,
                          int extractedFromRegister = -1) {
  ...
  UpdateMatrixSource(MatrixSlot_WV, shaderKey, baseRegister, rows, transposed,
                     manual, sourceLabel, extractedFromRegister);
}

static auto LoadSystemD3D9() -> HMODULE {
...
static auto LoadTargetD3D9() -> HMODULE {
  if (g_config.useRemixRuntime) {
    HMODULE remixModule = LoadLibraryA(g_config.remixDllName);
    if (remixModule != nullptr) {
      LogMsg("Loaded Remix runtime: %s", g_config.remixDllName);
      return remixModule;
    }
    LogMsg("WARNING: Failed to load Remix runtime '%s', falling back to system "
           "d3d9.dll",
           g_config.remixDllName);
  }
...
static auto GetWindowDpiScale(HWND hwnd) -> float {
  if (hwnd == nullptr) {
    return 1.0F;
  }
...
static void ApplyImGuiScale(HWND hwnd) {
  ...
  const float clampedUiScale =
      (std::max)(0.5F, (std::min)(3.0F, g_imguiScaleRuntime));
...
static void InitializeImGui(IDirect3DDevice9 *device, HWND hwnd) {
  if (g_imguiInitialized || (device == nullptr) || (hwnd == nullptr)) {
    return;
  }
  ...
  style.Colors[ImGuiCol_TabUnfocusedActive] =
      ImVec4(0.33F, 0.14F, 0.14F, 1.00F);
  ...
  if (g_imguiScaleRuntime < 0.5F) {
    g_imguiScaleRuntime = 0.5F;
}
  if (g_imguiScaleRuntime > 3.0F) {
    g_imguiScaleRuntime = 3.0F;
}
  ApplyImGuiScale(hwnd);

  g_imguiInitialized = true;
  g_imguiHwnd = hwnd;
  g_imguiPrevWndProc = reinterpret_cast<WNDPROC>(SetWindowLongPtr(
      hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(ImGuiWndProcHook)));
}
...
static void ShutdownImGui() {
  ...
  if ((g_imguiHwnd != nullptr) && (g_imguiPrevWndProc != nullptr)) {
    auto current =
        reinterpret_cast<WNDPROC>(GetWindowLongPtr(g_imguiHwnd, GWLP_WNDPROC));
    if (current == ImGuiWndProcHook) {
      SetWindowLongPtr(g_imguiHwnd, GWLP_WNDPROC,
...
static void DrawMatrix(const char *label, const D3DMATRIX &mat,
                       bool available) {
  if (!available) {
...
static auto HashBytesFNV1a(const uint8_t *data, size_t size) -> uint32_t {
...
static auto HashMatrix(const D3DMATRIX &mat) -> uint32_t {
  return HashBytesFNV1a(reinterpret_cast<const uint8_t *>(&mat),
                        sizeof(D3DMATRIX));
}

static auto GetShaderHashForKey(uintptr_t shaderKey) -> uint32_t {
  if (shaderKey == 0) {
    return 0;
  }
  uint32_t hash = 0;
  ...
  return HashBytesFNV1a(reinterpret_cast<const uint8_t *>(&shaderKey),
                        sizeof(shaderKey));
}

static void RecordConstantUpload(ConstantUploadStage stage, uintptr_t shaderKey,
                                 UINT startRegister, UINT vectorCount) {
...
static auto IsShaderDisabled(uintptr_t shaderKey) -> bool {
...
static auto IsCurrentShaderDrawDisabled(IDirect3DVertexShader9 *shader) -> bool {
  return IsShaderDisabled(reinterpret_cast<uintptr_t>(shader));
}

static auto GetShaderFlashStrength(const ShaderConstantState *state) -> float {
  if ((state == nullptr) || state->lastChangeSerial == 0 ||
      g_constantChangeSerial < state->lastChangeSerial) {
    return 0.0F;
  }
...
static void BuildShaderComboLabel(uintptr_t shaderKey, char *outLabel,
                                  size_t outSize) {
  if ((outLabel == nullptr) || outSize == 0) {
    return;
  }
  ...
  snprintf(outLabel, outSize, "%p (hash 0x%08X)%s%s%s",
           reinterpret_cast<void *>(shaderKey), displayHash,
           shaderKey == g_activeShaderKey ? " (active)" : "",
           disabled ? " [DISABLED]" : "", flash > 0.0F ? " [changed]" : "");
}
...
static void UpdateMatrixSource(MatrixSlot slot, uintptr_t shaderKey,
                               int baseRegister, int rows, bool transposed,
                               bool manual, const char *sourceLabel,
  ...
  info.sourceLabel = sourceLabel != nullptr ? sourceLabel
                                 : (manual ? "manual constants selection"
                                           : "auto/config detection");
  info.extractedFromRegister =
      extractedFromRegister >= 0 ? extractedFromRegister : baseRegister;
  std::lock_guard<std::mutex> lock(g_cameraMatricesMutex);
...
static void DrawMatrixSourceInfo(MatrixSlot slot, bool available) {
  ...
  uint32_t shaderHash = 0;
  bool hasBytecodeHash =
      TryGetShaderBytecodeHash(source.shaderKey, &shaderHash);
  if (source.shaderKey == 0) {
    ImGui::Text("Source shader: <none/runtime>");
  } else {
    ImGui::Text("Source shader: %p",
                reinterpret_cast<void *>(source.shaderKey));
    if (hasBytecodeHash) {
  ...
  if (source.baseRegister >= 0) {
    ...
    ImGui::Text("Stored from: c%d-c%d (%d rows)%s", source.baseRegister,
                source.baseRegister + (rows - 1), rows,
                source.transposed ? " [transposed]" : "");
  } else {
    ImGui::Text("Stored from: <not from shader constants>");
  }

  if (source.extractedFromRegister >= 0 &&
      source.extractedFromRegister != source.baseRegister) {
    ImGui::Text("Extracted from: c%d", source.extractedFromRegister);
  }

  ImGui::Text("Origin: %s",
              source.sourceLabel != nullptr ? source.sourceLabel : "unknown");
}

static auto CanAssignManualMatrix(MatrixSlot slot, char *reason,
                                  size_t reasonSize) -> bool {
  if (slot == MatrixSlot_View && g_iniViewMatrixRegister >= 0) {
    snprintf(reason, reasonSize,
             "Manual VIEW assignment blocked: ViewMatrixRegister is configured "
             "in camera_proxy.ini.");
    return false;
  }
  if (slot == MatrixSlot_Projection && g_iniProjMatrixRegister >= 0) {
    snprintf(reason, reasonSize,
             "Manual PROJECTION assignment blocked: ProjMatrixRegister is "
             "configured in camera_proxy.ini.");
    return false;
  }
  if (slot == MatrixSlot_World && g_iniWorldMatrixRegister >= 0) {
    snprintf(reason, reasonSize,
             "Manual WORLD assignment blocked: WorldMatrixRegister is "
             "configured in camera_proxy.ini.");
    return false;
  }
  if (slot == MatrixSlot_MVP && g_iniMvpMatrixRegister >= 0) {
    snprintf(reason, reasonSize,
             "Manual MVP assignment blocked: MVPMatrixRegister is configured "
             "in camera_proxy.ini.");
    return false;
  }
  if (slot == MatrixSlot_WV && g_iniMvMatrixRegister >= 0) {
    snprintf(reason, reasonSize,
             "Manual MV assignment blocked: MVMatrixRegister is configured in "
             "camera_proxy.ini.");
    return false;
  }
  if (slot == MatrixSlot_VP && g_iniVpMatrixRegister >= 0) {
    snprintf(reason, reasonSize,
             "Manual VP assignment blocked: VPMatrixRegister is configured in "
             "camera_proxy.ini.");
    return false;
  }
  return true;
...
static auto MatrixSlotLabel(MatrixSlot slot) -> const char * {
  switch (slot) {
...
static auto SaveConfigRegisterValue(const char *key, int value) -> bool {
  if (g_iniPath[0] == '\0') {
    return false;
}
  char valueBuf[32] = {};
  snprintf(valueBuf, sizeof(valueBuf), "%d", value);
  return WritePrivateProfileStringA("CameraProxy", key, valueBuf, g_iniPath) !=
         FALSE;
}

static auto SaveConfigBoolValue(const char *key, bool value) -> bool {
  return SaveConfigRegisterValue(key, value ? 1 : 0);
}

static auto SaveConfigFloatValue(const char *key, float value) -> bool {
  if (g_iniPath[0] == '\0') {
    return false;
}
  char valueBuf[64] = {};
...
static auto ConsumeSingleKeyHotkey(HotkeyAction action, int virtualKey) -> bool {
...
static void ResetMatrixRegisterOverridesToAuto() {
  ...
  bool savedAll =
      savedWorld && savedView && savedProj && savedMvp && savedMv && savedVp;
  if (g_config.autoDetectMatrices) {
    snprintf(g_matrixAssignStatus, sizeof(g_matrixAssignStatus),
             "Cleared matrix register overrides. Falling back to deterministic "
             "auto-detect (AutoDetectMatrices=1).%s",
             savedAll
                 ? ""
                 : " Failed to persist at least one key to camera_proxy.ini.");
  } else {
    snprintf(g_matrixAssignStatus, sizeof(g_matrixAssignStatus),
             "Cleared matrix register overrides. Runtime now uses structural "
             "detection.%s",
             savedAll
                 ? ""
                 : " Failed to persist at least one key to camera_proxy.ini.");
  }
  LogMsg("Matrix register overrides reset to auto (AutoDetectMatrices=%s).",
         g_config.autoDetectMatrices ? "on" : "off");
}
...
static void PinRegisterFromSource(MatrixSlot slot) {
  ...
  if (!source.valid || source.baseRegister < 0) {
    snprintf(g_matrixAssignStatus, sizeof(g_matrixAssignStatus),
             "Cannot pin %s: no register-backed source available.",
             MatrixSlotLabel(slot));
    return;
  }
...
static void TryAssignManualMatrixFromSelection(MatrixSlot slot,
  ...
  snprintf(g_matrixAssignStatus, sizeof(g_matrixAssignStatus),
           ...
           MatrixSlotLabel(slot), reinterpret_cast<void *>(shaderKey),
           baseRegister, baseRegister + rows - 1, rows);
}

static auto TransposeMatrix(const D3DMATRIX &mat) -> D3DMATRIX {
  D3DMATRIX out = {};
  out._11 = mat._11;
  out._12 = mat._21;
  out._13 = mat._31;
...
static void DrawMatrixWithTranspose(const char *label, const D3DMATRIX &mat,
                                    bool available, bool transpose) {
  if (!transpose) {
...
static auto GetShaderState(uintptr_t shaderKey,
  ...
  for (int & i : inserted.first->second.overrideExpiresAtFrame) {
    i = -1;
}
...
static void OnVertexShaderReleased(uintptr_t shaderKey) {
  ...
  if (recIt != g_shaderRecords.end()) {
    ...
    if (recIt->second.replacementShader != nullptr) {
  ...
  auto listIt = std::find(g_stableShaderList.begin(), g_stableShaderList.end(),
                          removedRecordPtr);
...
static void RegisterShaderBytecode(uintptr_t shaderKey, ShaderStageType stage,
                                   const std::vector<uint32_t> &tokens) {
  if (shaderKey == 0 || tokens.empty()) {
    return;
}
  ...
  rec.hash = HashBytesFNV1a(reinterpret_cast<const uint8_t *>(tokens.data()),
  ...
  snprintf(profile, sizeof(profile), "%s_%u_%u",
           stage == ShaderStage_Vertex ? "vs" : "ps", major, minor);
...
static auto ComputeShaderBytecodeHash(IDirect3DVertexShader9 *shader) -> uint32_t {
  if (shader == nullptr) {
    return 0;
  }
  UINT size = 0;
...
static auto TryBuildMatrix4x3FromSnapshot(const ShaderConstantState &state,
                                          int baseRegister, bool transposed,
                                          D3DMATRIX *outMatrix) -> bool {
  if ((outMatrix == nullptr) || !state.snapshotReady || baseRegister < 0 ||
      baseRegister + 2 >= kMaxConstantRegisters) {
    return false;
  }
  for (int i = 0; i < 3; i++) {
  ...
  D3DMATRIX m = {};
  if (!transposed) {
    m._11 = state.constants[baseRegister + 0][0];
    m._12 = state.constants[baseRegister + 0][1];
    m._13 = state.constants[baseRegister + 0][2];
    m._14 = state.constants[baseRegister + 0][3];
    m._21 = state.constants[baseRegister + 1][0];
    m._22 = state.constants[baseRegister + 1][1];
    m._23 = state.constants[baseRegister + 1][2];
    m._24 = state.constants[baseRegister + 1][3];
    m._31 = state.constants[baseRegister + 2][0];
    m._32 = state.constants[baseRegister + 2][1];
    m._33 = state.constants[baseRegister + 2][2];
    m._34 = state.constants[baseRegister + 2][3];
    m._41 = 0.0F;
    m._42 = 0.0F;
    m._43 = 0.0F;
    m._44 = 1.0F;
    ...
    m._11 = state.constants[baseRegister + 0][0];
    m._21 = state.constants[baseRegister + 0][1];
    m._31 = state.constants[baseRegister + 0][2];
    m._41 = state.constants[baseRegister + 0][3];
    m._12 = state.constants[baseRegister + 1][0];
    m._22 = state.constants[baseRegister + 1][1];
    m._32 = state.constants[baseRegister + 1][2];
    m._42 = state.constants[baseRegister + 1][3];
    m._13 = state.constants[baseRegister + 2][0];
    m._23 = state.constants[baseRegister + 2][1];
    m._33 = state.constants[baseRegister + 2][2];
    m._43 = state.constants[baseRegister + 2][3];
    m._14 = 0.0F;
    m._24 = 0.0F;
    m._34 = 0.0F;
    m._44 = 1.0F;
...
static auto InvertSimpleRigidView(const D3DMATRIX &view) -> D3DMATRIX {
  D3DMATRIX out = {};
  out._11 = view._11;
  out._12 = view._21;
  out._13 = view._31;
...
static auto ViewMatrixCanUseFastInverse(const D3DMATRIX &view) -> bool {
  const float row0len =
      sqrtf(Dot3(view._11, view._12, view._13, view._11, view._12, view._13));
  const float row1len =
      sqrtf(Dot3(view._21, view._22, view._23, view._21, view._22, view._23));
  const float row2len =
      sqrtf(Dot3(view._31, view._32, view._33, view._31, view._32, view._33));

  if (fabsf(row0len - 1.0F) > 0.05F || fabsf(row1len - 1.0F) > 0.05F ||
      fabsf(row2len - 1.0F) > 0.05F) {
    return false;
  }
  if (fabsf(Dot3(view._11, view._12, view._13, view._21, view._22, view._23)) >
      0.05F) {
    return false;
}
  if (fabsf(Dot3(view._11, view._12, view._13, view._31, view._32, view._33)) >
      0.05F) {
    return false;
}
  if (fabsf(Dot3(view._21, view._22, view._23, view._31, view._32, view._33)) >
      0.05F) {
    return false;
}
  if (fabsf(view._14) > 0.01F || fabsf(view._24) > 0.01F ||
      fabsf(view._34) > 0.01F || fabsf(view._44 - 1.0F) > 0.01F) {
    return false;
  }
  return true;
}

static auto TryBuildWorldFromView(const D3DMATRIX &view, bool preferFastInverse,
                                  D3DMATRIX *outWorld, bool *outUsedFast,
                                  bool *outFastEligible) -> bool {
  if (outWorld == nullptr) {
    return false;
  }

  const bool fastEligible = ViewMatrixCanUseFastInverse(view);
  if (outFastEligible != nullptr) {
    *outFastEligible = fastEligible;
  }

  if (preferFastInverse && fastEligible) {
    *outWorld = InvertSimpleRigidView(view);
    if (outUsedFast != nullptr) {
      *outUsedFast = true;
    }
    return true;
  }
...
static auto GameProfileLabel(GameProfileKind profile) -> const char * {
  switch (profile) {
  case GameProfile_Barnyard:
    return "Barnyard";
  case GameProfile_DevilMayCry4:
    return "DevilMayCry4";
  case GameProfile_MetalGearRising:
    return "MetalGearRising";
...
static auto ParseGameProfile(const char *profileName) -> GameProfileKind {
...
static auto InvertMatrix4x4Deterministic(const D3DMATRIX &in, D3DMATRIX *out,
                                         float *outDeterminant) -> bool {
  if (out == nullptr) {
    return false;
  }

  const auto *m = reinterpret_cast<const float *>(&in);
  float inv[16] = {};
...
static void ClearAllShaderOverrides() {
  for (auto &entry : g_shaderConstants) {
    ...
    for (int & i : state.overrideExpiresAtFrame) {
      i = -1;
}
  }
}
...
static void ClearShaderRegisterOverride(uintptr_t shaderKey, int reg) {
  ...
  memset(state->overrideConstants[reg], 0,
         sizeof(state->overrideConstants[reg]));
  state->overrideValid[reg] = false;
  state->overrideExpiresAtFrame[reg] = -1;
}

static auto BuildOverriddenConstants(ShaderConstantState &state,
                                     UINT startRegister, UINT vector4fCount,
                                     const float *sourceData,
                                     std::vector<float> &scratch) -> bool {
  if (!g_enableShaderEditing || (sourceData == nullptr) || vector4fCount == 0) {
    return false;
  }
  ...
  for (UINT i = 0; i < vector4fCount; i++) {
    ...
    memcpy(&scratch[i * 4], state.overrideConstants[reg],
           sizeof(state.overrideConstants[reg]));
...
static void UpdateVariance(ShaderConstantState &state, int reg,
                           const float *values) {
...
static auto GetVarianceMagnitude(const ShaderConstantState &state, int reg) -> float {
...
static auto
TryBuildMatrixFromConstantUpdate(const float *constantData, UINT startRegister,
                                 UINT vector4fCount, int baseRegister, int rows,
                                 bool transposed, D3DMATRIX *outMatrix) -> bool {
  if ((constantData == nullptr) || (outMatrix == nullptr) || rows < 3 || rows > 4) {
    return false;
  }
  if (baseRegister < 0) {
    return false;
...
static auto TryBuildMatrixSnapshot(const ShaderConstantState &state,
                                   int baseRegister, int rows, bool transposed,
                                   D3DMATRIX *outMatrix) -> bool {
  if ((outMatrix == nullptr) || !state.snapshotReady || baseRegister < 0 || rows < 3 ||
      rows > 4 || baseRegister + rows - 1 >= kMaxConstantRegisters) {
    return false;
  }
...
static auto TryBuildMatrixFromSnapshot(const ShaderConstantState &state,
                                       int baseRegister, D3DMATRIX *outMatrix) -> bool {
  return TryBuildMatrixSnapshot(state, baseRegister, 4, false, outMatrix);
}

static auto TryBuildMatrixSnapshotInfo(const ShaderConstantState &state,
                                       int baseRegister, D3DMATRIX *outMatrix,
                                       bool *looksLike) -> bool {
  if (!TryBuildMatrixFromSnapshot(state, baseRegister, outMatrix)) {
    if (looksLike != nullptr) {
      *looksLike = false;
    }
    return false;
  }
...
static auto TryBuildMatrixFromGlobalRegisters(int baseRegister, int rows,
                                              bool transposed,
                                              D3DMATRIX *outMatrix) -> bool {
  if ((outMatrix == nullptr) || baseRegister < 0 || rows < 3 || rows > 4 ||
  ...
  float m[16] = {};
  for (int i = 0; i < rows; i++) {
    const GlobalVertexRegisterState &globalState =
        g_allVertexRegisters[baseRegister + i];
...
static void ScanBuffer(const void *base, size_t size, int &resultsFound) {
  if ((base == nullptr) || size < sizeof(D3DMATRIX)) {
    return;
  }
  const auto *data = reinterpret_cast<const float *>(base);
  ...
  for (size_t i = 0; i + 16 <= count; i++) {
    ...
    snprintf(resultLine, sizeof(resultLine),
             "Memory scan: %s matrix at %p hash 0x%08X",
             looksView ? "VIEW" : "PROJ",
...
static DWORD WINAPI MemoryScannerThread(LPVOID lpParam) {
  std::string moduleName = lpParam != nullptr ? static_cast<const char *>(lpParam) : "";
  if (lpParam != nullptr) {
    free(lpParam);
  }
  ...
  while (true) {
    ...
    if ((((info.Protect & PAGE_EXECUTE_READWRITE) != 0u) ||
         ((info.Protect & PAGE_READWRITE) != 0u)) &&
...
static void StartMemoryScanner() {
  if (g_memoryScannerThread != nullptr) {
    return;
  }
  const char *moduleName =
      g_config.memoryScannerModule[0] != 0 ? g_config.memoryScannerModule : nullptr;
  {
    std::lock_guard<std::mutex> lock(g_uiDataMutex);
  ...
  g_memoryScannerThread = CreateThread(nullptr, 0, MemoryScannerThread,
                                       moduleCopy, 0, &g_memoryScannerThreadId);
  if (g_memoryScannerThread == nullptr) {
    LogMsg("WARNING: Failed to create memory scan thread.");
    if (moduleCopy != nullptr) {
      free(moduleCopy);
    }
  }
}
...
static void UpdateHotkeys() {
  ...
  if (ConsumeSingleKeyHotkey(HotkeyAction_ToggleMenu,
                             g_config.hotkeyToggleMenuVk)) {
    g_showImGui = !g_showImGui;
  }

  if (ConsumeSingleKeyHotkey(HotkeyAction_TogglePause,
                             g_config.hotkeyTogglePauseVk)) {
    g_pauseRendering = !g_pauseRendering;
  }

  if (ConsumeSingleKeyHotkey(HotkeyAction_EmitMatrices,
    ...
    if (!g_config.emitFixedFunctionTransforms) {
      snprintf(g_manualEmitStatus, sizeof(g_manualEmitStatus),
               "Blocked: set EmitFixedFunctionTransforms=1 in camera_proxy.ini "
               "first.");
    } else {
      g_requestManualEmit = true;
      snprintf(g_manualEmitStatus, sizeof(g_manualEmitStatus),
               "Pending (hotkey): pass cached World/View/Projection matrices "
               "to RTX Remix this frame.");
    }
  }

  if (ConsumeSingleKeyHotkey(HotkeyAction_ResetMatrixOverrides,
...
static void UpdateFrameTimeStats() {
  ...
  auto ms = static_cast<float>(delta * 1000.0);
  ...
  if (g_frameTimeSamples == 0) {
    ...
    if (ms < g_frameTimeMin) {
      g_frameTimeMin = ms;
}
    if (ms > g_frameTimeMax) {
      g_frameTimeMax = ms;
}
  }
  g_frameTimeSum += ms;
...
static void RenderImGuiOverlay(IDirect3DDevice9 *device) {
  ...
  if (g_showImGui) {
    POINT cursorScreen = {};
    if ((GetCursorPos(&cursorScreen) != 0) &&
        (ScreenToClient(g_imguiHwnd, &cursorScreen) != 0)) {
      ImGui::GetIO().MousePos = ImVec2(static_cast<float>(cursorScreen.x),
    ...
    static const int kNavKeys[] = {VK_TAB,    VK_LEFT,   VK_RIGHT,  VK_UP,
                                   VK_DOWN,   VK_PRIOR,  VK_NEXT,   VK_HOME,
  ...
  ImGui::Text("Game exe: %s", g_gameExeName[0] != 0 ? g_gameExeName : "<unknown>");
  ImGui::Text("Game name: %s",
              g_gameDisplayName[0] != 0 ? g_gameDisplayName : "<unknown>");
  ImGui::Text("Hotkeys: Toggle menu(F10) Pause(F9) Emit matrices(F8) Reset "
              "matrix overrides(F7)");
  if (ImGui::Button(g_pauseRendering ? "Resume game rendering"
                                     : "Pause game rendering")) {
    g_pauseRendering = !g_pauseRendering;
  }
  ImGui::SameLine();
  ImGui::Text("Status: %s", g_pauseRendering ? "Paused" : "Running");
  ImGui::TextWrapped("This proxy detects World, View, and Projection matrices "
                     "from shader constants and forwards them "
                     "to the RTX Remix runtime through SetTransform() so Remix "
                     "gets camera data in D3D9 titles.");

  if (ImGui::SliderFloat("UI scale", &g_imguiScaleRuntime, 0.5F, 3.0F,
                         "%.2fx")) {
    ApplyImGuiScale(g_imguiHwnd);
    g_config.imguiScale = g_imguiScaleRuntime;
  }

  if (ImGui::Checkbox("Disable game input while menu is open",
                      &g_imguiDisableGameInputWhileMenuOpen)) {
    g_config.disableGameInputWhileMenuOpen =
        g_imguiDisableGameInputWhileMenuOpen;
    SaveConfigBoolValue("DisableGameInputWhileMenuOpen",
                        g_config.disableGameInputWhileMenuOpen);
    UpdateInputBlockHooks();
  }
  ...
  if (ImGui::Button("Pass camera matrices to RTX Remix (SetTransform)")) {
    if (!g_config.emitFixedFunctionTransforms) {
      ...
      g_requestManualEmit = true;
      snprintf(g_manualEmitStatus, sizeof(g_manualEmitStatus),
               "Pending: pass cached World/View/Projection matrices to RTX "
               "Remix this frame.");
    }
  }
  ...
  if (g_showFpsStats && g_frameTimeSamples > 0) {
    ...
    float avgMs = g_frameTimeCount > 0
                      ? static_cast<float>(sumMs / g_frameTimeCount)
                      : 0.0F;
    ...
    ImGui::PushStyleColor(ImGuiCol_PlotLinesHovered,
                          ImVec4(1.00F, 1.00F, 1.00F, 1.00F));
    ImGui::PlotLines("Frame time (ms)", g_frameTimeHistory, g_frameTimeCount,
                     g_frameTimeIndex, nullptr, 0.0F, graphMaxMs,
                     ImVec2(0, 80));
  ...
  if (ImGui::BeginTabBar("MainTabs")) {
    if (ImGui::BeginTabItem("Camera")) {
      ImGui::Text("Active game profile: %s",
                  GameProfileLabel(g_activeGameProfile));
      ...
      if (g_gameSetTransformAnySeen) {
        if (ImGui::Checkbox(
                "Bypass proxy WVP emit when game provides SetTransform",
                &g_config.setTransformBypassProxyWhenGameProvides)) {
          SaveConfigBoolValue("SetTransformBypassProxyWhenGameProvides",
                              g_config.setTransformBypassProxyWhenGameProvides);
        }
        if (ImGui::Checkbox("Round-trip game SetTransform via GetTransform for "
        ...
        ImGui::TextDisabled("SetTransform compatibility options unlock once "
                            "game calls SetTransform(WORLD/VIEW/PROJECTION).");
      }
      if (g_activeGameProfile == GameProfile_MetalGearRising) {
        ImGui::Text(
            "MGR layout: Proj=c4-c7, ViewProjection=c8-c11, World=c16-c19");
        ImGui::Checkbox("Use auto projection when c4 is invalid",
                        &g_imguiMgrrUseAutoProjection);
        if (g_imguiMgrrUseAutoProjection !=
        ...
        ImGui::Text("Projection c4-c7 validity: %s",
                    g_mgrProjectionRegisterValid ? "valid" : "invalid");
        float activeViewDeterminant = 0.0F;
        D3DMATRIX activeViewInverse = {};
        InvertMatrix4x4Deterministic(camSnapshot.view, &activeViewInverse,
                                     &activeViewDeterminant);
        ImGui::Text("Determinant of active View matrix: %.6f",
                    activeViewDeterminant);
        ImGui::Text("Captured this frame: Proj=%s View=%s",
                    g_mgrProjCapturedThisFrame ? "yes" : "no",
                    g_mgrViewCapturedThisFrame ? "yes" : "no");
        ImGui::Text("Captured for draw: World=%s",
                    g_mgrWorldCapturedForDraw ? "yes" : "no");
        ...
        ImGui::Text("View source: %s",
                    g_profileViewDerivedFromInverse
                        ? "Derived from VP via inverse projection"
                        : "Not yet derived");
        if (g_profileStatusMessage[0] != '\0') {
          ImGui::TextWrapped("%s", g_profileStatusMessage);
        }
      } else if (g_activeGameProfile == GameProfile_DevilMayCry4) {
        ImGui::Text("DMC4 layout: MVP=c0-c3, World=c0-c3, View=c4-c7, "
                    "Projection=c8-c11");
        ...
        ImGui::Text("Barnyard profile: WORLD from VS constants; "
                    "VIEW/PROJECTION via intercepted game SetTransform");
        if (ImGui::Checkbox(
                "Use intercepted game View/Projection SetTransform",
                &g_imguiBarnyardUseGameSetTransformsForViewProjection)) {
          g_config.barnyardUseGameSetTransformsForViewProjection =
              g_imguiBarnyardUseGameSetTransformsForViewProjection;
          SaveConfigBoolValue(
              "BarnyardUseGameSetTransformsForViewProjection",
              g_config.barnyardUseGameSetTransformsForViewProjection);
        }
        if (ImGui::Checkbox("Always use c0-c3 as World",
                            &g_barnyardForceWorldFromC0)) {
          SaveConfigBoolValue("BarnyardForceWorldFromC0",
                              g_barnyardForceWorldFromC0);
        }
      ...
      if (ImGui::CollapsingHeader("Experimental inverse View -> World",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
        if (ImGui::Checkbox("Use inverse(View) as emitted World matrix",
                            &g_config.experimentalInverseViewAsWorld)) {
          SaveConfigBoolValue("ExperimentalInverseViewAsWorld",
                              g_config.experimentalInverseViewAsWorld);
        }
        if (ImGui::Checkbox(
                "Allow inverse(View) even if strict validity check fails",
                &g_config.experimentalInverseViewAsWorldAllowUnverified)) {
          SaveConfigBoolValue(
              "ExperimentalInverseViewAsWorldAllowUnverified",
              g_config.experimentalInverseViewAsWorldAllowUnverified);
        }
        if (ImGui::Checkbox("Fast inverse (rigid transform only)",
                            &g_config.experimentalInverseViewAsWorldFast)) {
          SaveConfigBoolValue("ExperimentalInverseViewAsWorldFast",
                              g_config.experimentalInverseViewAsWorldFast);
        }
        ImGui::TextWrapped(
            "Fast inverse assumes no scaling/shear: transpose the 3x3 rotation "
            "and recompute translation via negative dot products.");
        ImGui::Text("Last strict validity result: %s",
                    g_lastInverseViewAsWorldEligible ? "valid" : "invalid");
        ImGui::Text("Last inverse(View)->World application: %s",
                    g_lastInverseViewAsWorldApplied ? "applied"
                                                    : "not applied");
        ImGui::Text("Last method: %s", g_lastInverseViewAsWorldUsedFast
                                           ? "fast inverse"
                                           : "full 4x4 inverse");
      }
      ImGui::Separator();
      ...
      if (g_projectionDetectedByNumericStructure) {
        ...
        ImGui::Text("Handedness: %s",
                    ProjectionHandednessLabel(g_projectionDetectedHandedness));
        if (g_projectionDetectedRegister >= 0) {
          ImGui::Text("Detected register: c%d", g_projectionDetectedRegister);
        }
      } else {
        ImGui::Text(
            "Projection numeric detection: waiting for structural match");
      }

      if (ImGui::CollapsingHeader("Experimental custom projection",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
        ImGui::TextWrapped("This feature is experimental and must be enabled "
                           "in camera_proxy.ini. "
                           "By default it only supplies projection when no "
                           "register-sourced projection is available.");
        ImGui::Text("Enabled in ini: %s",
                    g_config.experimentalCustomProjectionEnabled ? "yes"
                                                                 : "no");
        if (!g_config.experimentalCustomProjectionEnabled) {
          ImGui::TextWrapped("Set ExperimentalCustomProjectionEnabled=1 in "
                             "camera_proxy.ini to activate this section.");
        } else {
          if (ImGui::RadioButton("Manual matrix",
                                 g_config.experimentalCustomProjectionMode ==
                                     CustomProjectionMode_Manual)) {
            g_config.experimentalCustomProjectionMode =
                CustomProjectionMode_Manual;
            SaveConfigRegisterValue("ExperimentalCustomProjectionMode",
                                    g_config.experimentalCustomProjectionMode);
          }
          ImGui::SameLine();
          if (ImGui::RadioButton("Auto-generate",
                                 g_config.experimentalCustomProjectionMode ==
                                     CustomProjectionMode_Auto)) {
            g_config.experimentalCustomProjectionMode =
                CustomProjectionMode_Auto;
          ...
          if (ImGui::Checkbox(
                  "Override combined MVP-derived projection",
          ...
          if (g_config.experimentalCustomProjectionMode ==
              CustomProjectionMode_Manual) {
            float rows[4][4] = {
                {g_config.experimentalCustomProjectionManualMatrix._11,
                 g_config.experimentalCustomProjectionManualMatrix._12,
                 g_config.experimentalCustomProjectionManualMatrix._13,
                 g_config.experimentalCustomProjectionManualMatrix._14},
                {g_config.experimentalCustomProjectionManualMatrix._21,
                 g_config.experimentalCustomProjectionManualMatrix._22,
                 g_config.experimentalCustomProjectionManualMatrix._23,
                 g_config.experimentalCustomProjectionManualMatrix._24},
                {g_config.experimentalCustomProjectionManualMatrix._31,
                 g_config.experimentalCustomProjectionManualMatrix._32,
                 g_config.experimentalCustomProjectionManualMatrix._33,
                 g_config.experimentalCustomProjectionManualMatrix._34},
                {g_config.experimentalCustomProjectionManualMatrix._41,
                 g_config.experimentalCustomProjectionManualMatrix._42,
                 g_config.experimentalCustomProjectionManualMatrix._43,
                 g_config.experimentalCustomProjectionManualMatrix._44}};
            bool edited = false;
            ...
            if (edited) {
              ...
              const auto *values = reinterpret_cast<const float *>(
                  &g_config.experimentalCustomProjectionManualMatrix);
              for (int i = 0; i < 16; ++i) {
                int row = (i / 4) + 1;
                int col = (i % 4) + 1;
                char key[64] = {};
                snprintf(key, sizeof(key), "ExperimentalCustomProjectionM%d%d",
                         row, col);
            ...
            if (ImGui::SliderFloat(
                    "Auto FOV (deg)",
                    &g_config.experimentalCustomProjectionAutoFovDeg, 1.0F,
                    179.0F, "%.2f")) {
              SaveConfigFloatValue(
                  "ExperimentalCustomProjectionAutoFovDeg",
                  g_config.experimentalCustomProjectionAutoFovDeg);
            }
            if (ImGui::InputFloat(
                    "Auto Near Z",
                    &g_config.experimentalCustomProjectionAutoNearZ, 0.01F,
                    0.1F, "%.6f")) {
              SaveConfigFloatValue(
            ...
            if (ImGui::InputFloat(
                    "Auto Far Z",
                    &g_config.experimentalCustomProjectionAutoFarZ, 1.0F, 10.0F,
                    "%.3f")) {
              SaveConfigFloatValue(
            ...
            if (ImGui::RadioButton("Left-handed", handednessIndex == 0)) {
              g_config.experimentalCustomProjectionAutoHandedness =
                  ProjectionHandedness_Left;
              SaveConfigRegisterValue(
                  "ExperimentalCustomProjectionAutoHandedness",
                  g_config.experimentalCustomProjectionAutoHandedness);
            }
            ImGui::SameLine();
      ...
      if (ImGui::CollapsingHeader("Combined matrix decomposition",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
        if (ImGui::Checkbox("Enable deterministic decomposition",
                            &g_config.enableCombinedDecomposition)) {
          SaveConfigBoolValue("EnableCombinedDecomposition",
                              g_config.enableCombinedDecomposition);
        }
        if (ImGui::Checkbox("Log decomposition",
                            &g_config.combinedDecompositionLog)) {
          SaveConfigBoolValue("CombinedDecompositionLog",
                              g_config.combinedDecompositionLog);
        }
        if (ImGui::Checkbox(
                "Allow generated/manual projection for VP decomposition",
                &g_config.allowGeneratedProjectionForVPDecomposition)) {
          SaveConfigBoolValue(
              "AllowGeneratedProjectionForVPDecomposition",
              g_config.allowGeneratedProjectionForVPDecomposition);
        }
        ImGui::TextWrapped("Formulas: inv(View)*MV=World, inv(World)*MV=View, "
                           "inv(Projection)*VP=View, inv(View)*VP=Projection, "
                           "inv(VP)*MVP=World, inv(MV)*MVP=Projection.");
        ImGui::Text("Decomposition attempted: %s",
                    g_combinedDecompDebug.attempted ? "yes" : "no");
        ImGui::Text("Solved World: %s",
                    g_combinedDecompDebug.solvedWorld ? "yes" : "no");
        if (g_combinedDecompDebug.worldFormula[0] != '\0') {
          ImGui::TextWrapped("  %s", g_combinedDecompDebug.worldFormula);
}
        ImGui::Text("Solved View: %s",
                    g_combinedDecompDebug.solvedView ? "yes" : "no");
        if (g_combinedDecompDebug.viewFormula[0] != '\0') {
          ImGui::TextWrapped("  %s", g_combinedDecompDebug.viewFormula);
}
        ImGui::Text("Solved Projection: %s",
                    g_combinedDecompDebug.solvedProjection ? "yes" : "no");
        if (g_combinedDecompDebug.projectionFormula[0] != '\0') {
          ImGui::TextWrapped("  %s", g_combinedDecompDebug.projectionFormula);
}
      }

      ImGui::Separator();
      ImGui::Text("Register pinning (camera tab)");
      ImGui::TextWrapped("Pin currently detected matrix registers directly "
                         "from this tab. Values are saved to camera_proxy.ini "
                         "immediately. Use reset to return to auto-detect.");
      ...
      ImGui::Text("Pinned registers: World=c%d View=c%d Projection=c%d MVP=c%d "
                  "MV=c%d VP=c%d",
    ...
    if (ImGui::BeginTabItem("Shader")) {
      ...
      static const char *kShaderListOrderModes[] = {"Insertion order",
                                                    "Hash (ascending)"};
      ImGui::InputText("Filter", shaderFilter, sizeof(shaderFilter));
      ImGui::SameLine();
      ImGui::SetNextItemWidth(180.0F);
      ImGui::Combo("Order", &g_shaderListOrderMode, kShaderListOrderModes,
                   IM_ARRAYSIZE(kShaderListOrderModes));

      std::vector<ShaderRecord *> shaderSnapshot = g_stableShaderList;
      if (g_shaderListOrderMode == ShaderListOrder_HashAsc) {
        std::sort(shaderSnapshot.begin(), shaderSnapshot.end(),
                  [](const ShaderRecord *a, const ShaderRecord *b) {
      ...
      if (selectedByKeyIt != g_shaderRecords.end()) {
        g_selectedShaderHash =
            static_cast<uint64_t>(selectedByKeyIt->second.hash);
      } else if (g_selectedShaderHash != 0) {
        auto selectedIt =
            g_shaderHashToKey.find(static_cast<uint32_t>(g_selectedShaderHash));
        if (selectedIt != g_shaderHashToKey.end()) {
          g_selectedShaderKey = selectedIt->second;
}
      }
      if (g_selectedShaderHash == 0 && !shaderSnapshot.empty() &&
          (shaderSnapshot.front() != nullptr)) {
        g_selectedShaderHash =
            static_cast<uint64_t>(shaderSnapshot.front()->hash);
        g_selectedShaderKey = shaderSnapshot.front()->shaderKey;
      }

      ImGui::Columns(3, "ShaderCols", true);
      ImGui::BeginChild("ShaderBrowser", ImVec2(0, 420), 1);
      for (ShaderRecord *recPtr : shaderSnapshot) {
        ...
        snprintf(visibleLabel, sizeof(visibleLabel),
                 "0x%08X %s %s inst:%d use:%llu%s", rec.hash,
                 rec.stage == ShaderStage_Vertex ? "VS" : "PS",
                 rec.shaderModel.c_str(), static_cast<int>(rec.ir.size()),
                 rec.usageCount,
                 (shaderKey == g_activeVertexShaderKey ||
                  shaderKey == g_activePixelShaderKey)
                     ? " *"
                     : "");
        if ((shaderFilter[0] != 0) && strstr(visibleLabel, shaderFilter) == nullptr) {
          continue;
}
        bool selected = (shaderKey == g_selectedShaderKey);
        char selectableLabel[320];
        snprintf(selectableLabel, sizeof(selectableLabel), "%s###shader_row_%p",
                 visibleLabel, reinterpret_cast<void *>(shaderKey));
      ...
      ImGui::NextColumn();
      ImGui::BeginChild("ShaderInspect", ImVec2(0, 420), 1);
      auto it = g_shaderRecords.find(g_selectedShaderKey);
      if (it != g_shaderRecords.end() &&
          static_cast<uint64_t>(it->second.hash) == g_selectedShaderHash) {
        ...
        bool replacementEnabled =
            rec.replacementEnabled && rec.replacementShader != nullptr;
        if (ImGui::Checkbox("Enable replacement", &replacementEnabled)) {
          rec.replacementEnabled =
              replacementEnabled && rec.replacementShader != nullptr;
          if (replacementEnabled && (rec.replacementShader == nullptr)) {
            rec.replacementStatus =
                "No replacement object yet. Use Replace shader first.";
          }
        }
        ...
        if (ImGui::Button("Dump assembly")) {
          ...
          char path[128];
          snprintf(path, sizeof(path), "Shader_dump/%08X.asm.txt", rec.hash);
        ...
        if (ImGui::Button("Dump bytecode")) {
          std::filesystem::create_directories("Shader_dump");
          char path[128];
          snprintf(path, sizeof(path), "Shader_dump/%08X.bytecode.bin",
                   rec.hash);
          std::ofstream f(path, std::ios::binary);
          const std::vector<uint32_t> &data =
              rec.replacementEnabled && !rec.modifiedBytecode.empty()
                  ? rec.modifiedBytecode
                  : rec.originalBytecode;
        ...
        if (g_shaderEditorKey != rec.shaderKey) {
          ...
          strncpy_s(g_shaderEditorBuffer.data(), g_shaderEditorBuffer.size(),
                    rec.editableAssembly.c_str(), _TRUNCATE);
        }
        ImGuiInputTextFlags flags = ImGuiInputTextFlags_AllowTabInput;
        if (ImGui::InputTextMultiline(
                "##ShaderAssemblyEditor", g_shaderEditorBuffer.data(),
                g_shaderEditorBuffer.size(), ImVec2(-1, 180), flags)) {
        ...
        if (ImGui::Button("Replace shader")) {
          ...
          if (BuildShaderReplacementFromAssembly(device, &rec, &err)) {
            rec.replacementEnabled = true;
            rec.replacementStatus =
                "Replacement shader created and active for runtime binding.";
          } else {
            rec.replacementStatus =
                err.empty() ? "Failed to build replacement shader." : err;
          }
        }
        ImGui::SameLine();
        if (ImGui::Button("Reset assembly")) {
          ...
          rec.replacementStatus =
              "Assembly reset to original and replacement shader released.";
        }
      ...
      if (ai != g_shaderRecords.end() &&
        ...
        const int effectiveTransformBase = g_manualTransformBaseOverride >= 0
                                               ? g_manualTransformBaseOverride
                                               : rec.transformConstantBase;
        const int effectiveLightingBase = g_manualLightingBaseOverride >= 0
        ...
        ImGui::Text("POSITION: %s",
                    g_activeVertexDeclInfo.hasPosition ? "Yes" : "No");
        ImGui::Text("NORMAL: %s",
                    g_activeVertexDeclInfo.hasNormal ? "Yes" : "No");
        ImGui::Text("TEXCOORDS: %d", g_activeVertexDeclInfo.texcoordCount);
        ImGui::Text("TANGENT: %s",
                    g_activeVertexDeclInfo.hasTangent ? "Yes" : "No");
        ImGui::Text("BINORMAL: %s",
                    g_activeVertexDeclInfo.hasBinormal ? "Yes" : "No");
        ...
        if (ImGui::Button("Jump to transform register") &&
            effectiveTransformBase >= 0) {
          g_selectedRegister = effectiveTransformBase;
        }
        ImGui::SameLine();
        if (ImGui::Button("Jump to lighting register") &&
            effectiveLightingBase >= 0) {
          g_selectedRegister = effectiveLightingBase;
        }
        ImGui::Separator();
        ImGui::Checkbox("Force FFP transform emission", &g_forceFfpTransform);
        ImGui::InputInt("Manual transform base",
                        &g_manualTransformBaseOverride);
        ImGui::InputInt("Manual lighting base", &g_manualLightingBaseOverride);

        ImGui::Separator();
        ImGui::Text("Constant monitor (selected shader)");
        ImGui::BeginChild("ShaderConstRange", ImVec2(-1, 145), 1);
        for (int reg = usedMin; reg >= 0 && reg <= usedMax; ++reg) {
          if (!rec.constantUsage[reg]) {
            continue;
}
          bool isTransformReg = effectiveTransformBase >= 0 &&
                                reg >= effectiveTransformBase &&
                                reg < (effectiveTransformBase + 4);
          bool isLightingReg =
              (reg == effectiveLightingBase && effectiveLightingBase >= 0);
          if (isTransformReg) {
            ImGui::PushStyleColor(ImGuiCol_Text,
                                  ImVec4(0.96F, 0.40F, 0.40F, 1.00F));
          } else if (isLightingReg) {
            ImGui::PushStyleColor(ImGuiCol_Text,
                                  ImVec4(0.98F, 0.72F, 0.42F, 1.00F));
          }
          if (ImGui::Selectable(
                  (std::string("c") + std::to_string(reg)).c_str(),
                  g_selectedRegister == reg)) {
            g_selectedRegister = reg;
          }
          if (isTransformReg || isLightingReg) {
            ImGui::PopStyleColor();
}
        }
        ImGui::EndChild();
    ...
    if (ImGui::BeginTabItem("Constants")) {
          ...
          for (uintptr_t key : g_shaderOrder) {
            ...
            if (flashStrength > 0.0F) {
              ImGui::PushStyleColor(
                  ImGuiCol_Text, ImVec4(1.00F, 0.35F + 0.45F * flashStrength,
                                        0.35F + 0.45F * flashStrength, 1.00F));
            }
      ...
      ImGui::Checkbox("View all VS constant registers (all shaders)",
                      &g_showAllConstantRegisters);

      ImGui::Checkbox("Group by 4-register matrices",
                      &g_showConstantsAsMatrices);
      ImGui::SameLine();
      ...
      ImGui::Combo("Override scope", &g_overrideScopeMode, kOverrideModes,
                   IM_ARRAYSIZE(kOverrideModes));
      if (g_overrideScopeMode == Override_NFrames) {
        ImGui::InputInt("Override frames", &g_overrideNFrames);
        if (g_overrideNFrames < 1) {
          g_overrideNFrames = 1;
}
      }

      ShaderConstantState *editState =
          GetShaderState(g_selectedShaderKey, false);
      if (g_selectedRegister >= 0) {
        ImGui::Text("Selected register: c%d", g_selectedRegister);
        if ((editState != nullptr) && g_selectedRegister < kMaxConstantRegisters) {
          float editValues[4] = {};
          if (editState->overrideValid[g_selectedRegister]) {
            memcpy(editValues, editState->overrideConstants[g_selectedRegister],
                   sizeof(editValues));
          } else if (editState->valid[g_selectedRegister]) {
            memcpy(editValues, editState->constants[g_selectedRegister],
                   sizeof(editValues));
          }
          ...
          if (ImGui::InputFloat4("Override values", editValues, "%.6f")) {
            ...
            if (g_overrideScopeMode == Override_OneFrame) {
              editState->overrideExpiresAtFrame[g_selectedRegister] =
                  g_frameCount + 1;
            } else if (g_overrideScopeMode == Override_NFrames) {
              editState->overrideExpiresAtFrame[g_selectedRegister] =
                  g_frameCount + g_overrideNFrames;
            } else {
          ...
          if (ImGui::Button("Reset selected override")) {
            ClearShaderRegisterOverride(g_selectedShaderKey,
                                        g_selectedRegister);
          }
      ...
      ImGui::BeginChild("ConstantsScroll", ImVec2(0, 270), 1);
      ...
      if (g_showAllConstantRegisters) {
        ...
        if (g_showConstantsAsMatrices) {
          for (int base = 0; base < kMaxConstantRegisters; base += 4) {
            ...
            bool looksLike =
                hasMatrix &&
                LooksLikeMatrix(reinterpret_cast<const float *>(&mat));
            ...
            if (open) {
              for (int reg = base; reg < base + 4; reg++) {
                char rowLabel[192] = {};
                const GlobalVertexRegisterState &globalState =
                    g_allVertexRegisters[reg];
                if (globalState.valid) {
                  snprintf(rowLabel, sizeof(rowLabel),
                           "c%d: [%.3f %.3f %.3f %.3f]###all_reg_%d", reg,
                           globalState.value[0], globalState.value[1],
                           globalState.value[2], globalState.value[3], reg);
                } else {
                  snprintf(rowLabel, sizeof(rowLabel),
                           "c%d: <unset>###all_reg_%d", reg, reg);
                }
              ...
              if (canAssign) {
                uintptr_t sourceShaderKey =
                    g_allVertexRegisters[base].lastShaderKey;
                ImGui::PushID(base + 5000);
                if (ImGui::Button("Use as World")) {
                  TryAssignManualMatrixFromSelection(MatrixSlot_World,
                                                     sourceShaderKey, base,
                                                     selectedRows, mat);
                }
                ImGui::SameLine();
                if (ImGui::Button("Use as View")) {
                  TryAssignManualMatrixFromSelection(MatrixSlot_View,
                                                     sourceShaderKey, base,
                                                     selectedRows, mat);
                }
                ImGui::SameLine();
                if (ImGui::Button("Use as Projection")) {
                  TryAssignManualMatrixFromSelection(MatrixSlot_Projection,
                                                     sourceShaderKey, base,
                                                     selectedRows, mat);
                }
                ImGui::SameLine();
                if (ImGui::Button("Use as MVP")) {
                  TryAssignManualMatrixFromSelection(
                      MatrixSlot_MVP, sourceShaderKey, base, selectedRows, mat);
                }
                ImGui::PopID();
              }
              ImGui::TreePop();
      ...
      } else if ((state != nullptr) && state->snapshotReady) {
        if (g_showConstantsAsMatrices) {
          for (int base = 0; base < kMaxConstantRegisters; base += 4) {
            D3DMATRIX mat = {};
            bool looksLike = false;
            bool hasMatrix =
                TryBuildMatrixSnapshotInfo(*state, base, &mat, &looksLike);
            ...
            if (open) {
              ...
              for (int reg = base; reg < base + 4; reg++) {
                ...
                if (state->valid[reg]) {
                  if (g_showTransposedMatrices && hasMatrix) {
                    int row = reg - base;
                    const float *data =
                        reinterpret_cast<const float *>(&displayMat) + row * 4;
                    snprintf(rowLabel, sizeof(rowLabel),
                             "r%d: [%.3f %.3f %.3f %.3f]###reg_%d", row,
                             data[0], data[1], data[2], data[3], reg);
                  } else {
                    const float *data = state->constants[reg];
                    snprintf(rowLabel, sizeof(rowLabel),
                             "c%d: [%.3f %.3f %.3f %.3f]###reg_%d", reg,
                             data[0], data[1], data[2], data[3], reg);
                  }
                } else {
                  snprintf(rowLabel, sizeof(rowLabel), "c%d: <unset>###reg_%d",
                           reg, reg);
                }
              ...
              bool canAssign = state->valid[base] && state->valid[base + 1] &&
                               state->valid[base + 2] &&
                               (selectedRows == 3 || state->valid[base + 3]);
              if (canAssign) {
                D3DMATRIX assignedMat = {};
                if (!TryBuildMatrixSnapshot(*state, base, selectedRows, false,
                                            &assignedMat)) {
                  canAssign = false;
                }
                if (canAssign) {
                  ...
                  if (ImGui::Button("Use as World")) {
                    TryAssignManualMatrixFromSelection(
                        MatrixSlot_World, g_selectedShaderKey, base,
                        selectedRows, assignedMat);
                  }
                  ImGui::SameLine();
                  if (ImGui::Button("Use as View")) {
                    TryAssignManualMatrixFromSelection(
                        MatrixSlot_View, g_selectedShaderKey, base,
                        selectedRows, assignedMat);
                  }
                  ImGui::SameLine();
                  if (ImGui::Button("Use as Projection")) {
                    TryAssignManualMatrixFromSelection(
                        MatrixSlot_Projection, g_selectedShaderKey, base,
                        selectedRows, assignedMat);
                  }
                  ImGui::SameLine();
                  if (ImGui::Button("Use as MVP")) {
                    TryAssignManualMatrixFromSelection(
                        MatrixSlot_MVP, g_selectedShaderKey, base, selectedRows,
                        assignedMat);
                  }
                  ImGui::PopID();
                }
          ...
          for (int reg = 0; reg < kMaxConstantRegisters; reg++) {
            ...
            char rowLabel[128];
            snprintf(rowLabel, sizeof(rowLabel),
                     "c%d: [%.3f %.3f %.3f %.3f]###reg_%d", reg, data[0],
                     data[1], data[2], data[3], reg);
            ImGui::PushID(reg);
            if (ImGui::Selectable(rowLabel, g_selectedRegister == reg)) {
              g_selectedRegister = reg;
            }
    ...
    if (ImGui::BeginTabItem("Memory Scanner")) {
      ...
      ImGui::SameLine();
      ImGui::Text("Status: %s", g_memoryScannerThread != nullptr ? "running" : "idle");
      ImGui::SameLine();
      ...
      ImGui::BeginChild("MemoryScanResults", ImVec2(0, 360), 1);
      {
        std::lock_guard<std::mutex> lock(g_uiDataMutex);
          ...
          for (size_t i = 0; i < g_memoryScanHits.size(); ++i) {
            ...
            if (ImGui::Button("Use as Projection")) {
              StoreProjectionMatrix(hit.matrix, 0, -1, 4, false, true,
                                    "memory scanner");
              snprintf(
                  g_matrixAssignStatus, sizeof(g_matrixAssignStatus),
    ...
    if (ImGui::BeginTabItem("Logs")) {
      ...
      if (g_logsLiveUpdate && g_logSnapshotDirty) {
        RefreshLogSnapshot();
      } else if (!g_logsLiveUpdate && g_logSnapshot.empty() &&
                 g_logSnapshotDirty) {
        RefreshLogSnapshot();
      }
      ImGui::Separator();
      ImGui::BeginChild("FormattedLogs", ImVec2(0, 380), 1);
...
void LogMsg(const char *fmt, ...) {
  ...
  if (!g_config.enableLogging || (g_logFile == nullptr)) {
    return;
}
...
static auto Determinant3x3(const D3DMATRIX &m) -> float {
...
static auto LooksLikeViewStrict(const D3DMATRIX &m) -> bool {
  ...
  if (fabsf(row0len - 1.0F) > 0.05F) {
    return false;
}
  if (fabsf(row1len - 1.0F) > 0.05F) {
    return false;
}
  if (fabsf(row2len - 1.0F) > 0.05F) {
    return false;
}

  if (fabsf(Dot3(m._11, m._12, m._13, m._21, m._22, m._23)) > 0.05F) {
    return false;
}
  if (fabsf(Dot3(m._11, m._12, m._13, m._31, m._32, m._33)) > 0.05F) {
    return false;
}
  if (fabsf(Dot3(m._21, m._22, m._23, m._31, m._32, m._33)) > 0.05F) {
    return false;
}

  if (fabsf(m._14) > 0.01F || fabsf(m._24) > 0.01F || fabsf(m._34) > 0.01F) {
    return false;
}
  if (fabsf(m._44 - 1.0F) > 0.01F) {
    return false;
}

  float det = Determinant3x3(m);
  return fabsf(det - 1.0f) <= 0.1f;
}
...
static auto IsTypicalProjectionMatrix(const D3DMATRIX &m) -> bool {
  ProjectionAnalysis analysis = {};
  if (!AnalyzeProjectionMatrixNumeric(m, &analysis) || !analysis.valid) {
    return false;
  }
  if (analysis.fovRadians < g_config.minFOV ||
      analysis.fovRadians > g_config.maxFOV) {
    return false;
  }

  const bool perspectiveTermsLookValid =
      fabsf(m._14) <= 0.05F && fabsf(m._24) <= 0.05F && fabsf(m._44) <= 0.05F &&
...
static auto AnalyzeProjectionMatrixNumeric(const D3DMATRIX &m,
  ...
  if (out != nullptr) {
    out->valid = true;
    out->fovRadians = fov;
    out->handedness = (m._34 >= 0.0F) ? ProjectionHandedness_Left
                                      : ProjectionHandedness_Right;
  }
  return true;
}
...
static auto ProjectionHandednessLabel(ProjectionHandedness handedness) -> const char * {
  switch (handedness) {
  case ProjectionHandedness_Left:
    return "LH";
  case ProjectionHandedness_Right:
    return "RH";
  default:
    return "Unknown";
  }
...
static auto HasPerspectiveComponent(const D3DMATRIX &m) -> bool {
  return fabsf(m._34) > 0.5F && fabsf(m._44) < 0.5F;
}

static auto IsAffineMatrixNoPerspective(const D3DMATRIX &m) -> bool {
  return fabsf(m._14) < 0.02F && fabsf(m._24) < 0.02F && fabsf(m._34) < 0.02F &&
         fabsf(m._44 - 1.0F) < 0.02F;
}

static auto IsLikelyBoneTransform(const D3DMATRIX &m, int rows,
                                  UINT vectorCountInUpload, UINT uploadStartReg,
                                  UINT candidateBaseReg) -> bool {
  ...
  const bool nearUnitRows = fabsf(r0 - 1.0F) < 0.25F &&
                            fabsf(r1 - 1.0F) < 0.25F &&
                            fabsf(r2 - 1.0F) < 0.25F;

  const bool appearsInPaletteUpload =
      vectorCountInUpload >= 8 && candidateBaseReg >= uploadStartReg &&
      (candidateBaseReg + static_cast<UINT>(rows)) <=
          (uploadStartReg + vectorCountInUpload);

  return nearUnitRows && appearsInPaletteUpload;
}

static auto LooksLikeWorldStrict(const D3DMATRIX &m, int rows,
                                 UINT vectorCountInUpload, UINT uploadStartReg,
                                 UINT candidateBaseReg) -> bool {
  if (!IsAffineMatrixNoPerspective(m)) {
    return false;
  ...
  if (!std::isfinite(det) || fabsf(det) < 0.0001F) {
    return false;
}

  if (IsLikelyBoneTransform(m, rows, vectorCountInUpload, uploadStartReg,
                            candidateBaseReg)) {
    return false;
  }
  return true;
...
static auto
ClassifyMatrixDeterministic(const D3DMATRIX &m, int rows,
                            UINT vectorCountInUpload, UINT uploadStartReg,
                            UINT candidateBaseReg) -> MatrixClassification {
  ...
  if (LooksLikeWorldStrict(m, rows, vectorCountInUpload, uploadStartReg,
                           candidateBaseReg)) {
...
static auto IsThreeRowPrefixOfPerspectiveMatrix(const float *data,
                                                UINT startReg, UINT vectorCount,
                                                UINT candidateBaseReg,
                                                bool transposedLayout) -> bool;

static auto CountStridedCandidates(const float *data, UINT startReg,
                                  UINT vectorCount, UINT strideRows,
                                  MatrixClassification targetClass) -> int {
  int count = 0;
  for (UINT offset = 0; offset + strideRows <= vectorCount;
       offset += strideRows) {
    D3DMATRIX candidate = {};
    if (!TryBuildMatrixFromConstantUpdate(
            data + offset * 4, startReg + offset, strideRows,
            static_cast<int>(startReg + offset), static_cast<int>(strideRows),
            false, &candidate)) {
      continue;
    }
    MatrixClassification cls =
        ClassifyMatrixDeterministic(candidate, static_cast<int>(strideRows),
                                    vectorCount, startReg, startReg + offset);

    if (strideRows == 3U &&
        (cls == MatrixClass_View || cls == MatrixClass_World) &&
        IsThreeRowPrefixOfPerspectiveMatrix(data, startReg, vectorCount,
                                            startReg + offset, false)) {
      continue;
    }
...
static auto IsThreeRowPrefixOfPerspectiveMatrix(const float *data,
  ...
  if (!TryBuildMatrixFromConstantUpdate(data + offset * 4, candidateBaseReg, 4U,
                                        static_cast<int>(candidateBaseReg), 4,
                                        transposedLayout, &candidate4x4)) {
    return false;
  }

  const MatrixClassification directClass = ClassifyMatrixDeterministic(
      candidate4x4, 4, vectorCount, startReg, candidateBaseReg);
  if (directClass == MatrixClass_Projection ||
      directClass == MatrixClass_CombinedPerspective) {
    return true;
  }
  ...
  const MatrixClassification transposedClass = ClassifyMatrixDeterministic(
      transposed, 4, vectorCount, startReg, candidateBaseReg);
  return transposedClass == MatrixClass_Projection ||
         transposedClass == MatrixClass_CombinedPerspective;
}

static auto MultiplyMatrix(const D3DMATRIX &a, const D3DMATRIX &b) -> D3DMATRIX {
  D3DMATRIX out = {};
...
static auto
CrossValidateViewAgainstProjection(const D3DMATRIX &candidateView,
                                   const D3DMATRIX &knownProjection) -> bool {
  D3DMATRIX vp = MultiplyMatrix(candidateView, knownProjection);

  const float p_c0 = sqrtf(knownProjection._11 * knownProjection._11 +
                           knownProjection._21 * knownProjection._21 +
                           knownProjection._31 * knownProjection._31);
  const float p_c1 = sqrtf(knownProjection._12 * knownProjection._12 +
                           knownProjection._22 * knownProjection._22 +
                           knownProjection._32 * knownProjection._32);
  const float vp_c0 =
      sqrtf(vp._11 * vp._11 + vp._21 * vp._21 + vp._31 * vp._31);
  const float vp_c1 =
      sqrtf(vp._12 * vp._12 + vp._22 * vp._22 + vp._32 * vp._32);
...
static auto IsIdentityMatrix(const D3DMATRIX &m, float tolerance) -> bool {
  return fabsf(m._11 - 1.0F) < tolerance && fabsf(m._22 - 1.0F) < tolerance &&
         fabsf(m._33 - 1.0F) < tolerance && fabsf(m._44 - 1.0F) < tolerance &&
         fabsf(m._12) < tolerance && fabsf(m._13) < tolerance &&
         fabsf(m._14) < tolerance && fabsf(m._21) < tolerance &&
         fabsf(m._23) < tolerance && fabsf(m._24) < tolerance &&
         fabsf(m._31) < tolerance && fabsf(m._32) < tolerance &&
         fabsf(m._34) < tolerance && fabsf(m._41) < tolerance &&
         fabsf(m._42) < tolerance && fabsf(m._43) < tolerance;
}
static auto MatrixIdentityMaxError(const D3DMATRIX &m) -> float {
  const D3DMATRIX identity = {1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F,
                              0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F};
  const auto *a = reinterpret_cast<const float *>(&m);
  const auto *b = reinterpret_cast<const float *>(&identity);
  float maxErr = 0.0F;
  for (int i = 0; i < 16; ++i) {
...
static auto MatrixClose(const D3DMATRIX &a, const D3DMATRIX &b,
                        float tolerance) -> bool {
  const auto *pa = reinterpret_cast<const float *>(&a);
  const auto *pb = reinterpret_cast<const float *>(&b);
  for (int i = 0; i < 16; i++) {
...
auto LooksLikeProjection(const D3DMATRIX &m) -> bool {
  return AnalyzeProjectionMatrixNumeric(m, nullptr);
}

// Create a standard perspective projection matrix
void CreateProjectionMatrix(D3DMATRIX *out, float fovY, float aspect,
...
static void
CreateProjectionMatrixWithHandedness(D3DMATRIX *out, float fovY, float aspect,
                                     float zNear, float zFar,
...
static auto TryGetCurrentDisplayAspect(IDirect3DDevice9 *device, HWND hwnd,
                                       float *outAspect, UINT *outWidth,
                                       UINT *outHeight) -> bool {
  if (outAspect == nullptr) {
    return false;
  }

  if (device != nullptr) {
    D3DVIEWPORT9 viewport = {};
    if (SUCCEEDED(device->GetViewport(&viewport)) && viewport.Width > 0 &&
        viewport.Height > 0) {
      *outAspect = static_cast<float>(viewport.Width) /
...
static auto BuildExperimentalCustomProjectionMatrix(
    IDirect3DDevice9 *device, HWND hwnd, D3DMATRIX *outProjection,
    bool *outUsedAuto, float *outResolvedAspect, UINT *outWidth,
    UINT *outHeight) -> bool {
  if ((outProjection == nullptr) || !g_config.experimentalCustomProjectionEnabled) {
    return false;
  }
  ...
  const float farZ =
      (std::max)(nearZ + 0.001F, g_config.experimentalCustomProjectionAutoFarZ);
  ...
  CreateProjectionMatrixWithHandedness(outProjection,
                                       ...
                                       safeAspect, nearZ, farZ, handedness);

  if (outUsedAuto != nullptr) {
    *outUsedAuto = true;
  }
  if (outResolvedAspect != nullptr) {
...
static auto TryExtractProjectionFromCombined(const D3DMATRIX &combined,
                                             const D3DMATRIX *worldOptional,
                                             bool worldAvailable,
                                             ProjectionAnalysis *outAnalysis,
                                             D3DMATRIX *outProjection,
                                             bool /*forceDecomposition*/) -> bool {
  if (outProjection == nullptr) {
    return false;
}
  D3DMATRIX candidate = combined;
  if (worldAvailable && (worldOptional != nullptr)) {
    D3DMATRIX worldInv = {};
  ...
  if (!AnalyzeProjectionMatrixNumeric(candidate, outAnalysis)) {
    return false;
}
...
static auto TrySolveFromInverseLeftMultiply(const D3DMATRIX &known,
...
static auto TrySolveFromInverseRightMultiply(const D3DMATRIX &combined,
...
static auto IsFiniteMatrix(const D3DMATRIX &m) -> bool {
  const auto *v = reinterpret_cast<const float *>(&m);
  for (int i = 0; i < 16; ++i) {
    if (!std::isfinite(v[i])) {
      return false;
}
...
static auto IsWorldCandidateForDecomposition(const D3DMATRIX &m) -> bool {
  if (!IsFiniteMatrix(m) || !IsAffineMatrixNoPerspective(m)) {
    return false;
}
  const float det = Determinant3x3(m);
...
static auto IsViewCandidateForDecomposition(const D3DMATRIX &m) -> bool {
  return IsFiniteMatrix(m) && LooksLikeViewStrict(m);
}

static auto IsProjectionCandidateForDecomposition(const D3DMATRIX &m) -> bool {
  return IsFiniteMatrix(m) && LooksLikeProjectionStrict(m);
}

static void TryDecomposeCombinedMatricesDeterministic(
    D3DMATRIX *world, bool *hasWorld, D3DMATRIX *view, bool *hasView,
    D3DMATRIX *projection, bool *hasProjection, const D3DMATRIX *mvp,
    bool hasMvp, const D3DMATRIX *mv, bool hasMv, const D3DMATRIX *vp,
    bool hasVp, const D3DMATRIX *generatedProjection,
    bool hasGeneratedProjection) {
  if ((world == nullptr) || (hasWorld == nullptr) || (view == nullptr) || (hasView == nullptr) || (projection == nullptr) || (hasProjection == nullptr)) {
    return;
}

  g_combinedDecompDebug = {};
  g_combinedDecompDebug.attempted = hasMvp || hasMv || hasVp;
  if (!g_config.enableCombinedDecomposition ||
      !g_combinedDecompDebug.attempted) {
    return;
  }
  ...
  if (knownHasMvp && (mvp != nullptr)) {
    knownMvp = *mvp;
}
  if (knownHasMv && (mv != nullptr)) {
    knownMv = *mv;
}
  if (knownHasVp && (vp != nullptr)) {
    knownVp = *vp;
}

  auto solveWorld = [&](const D3DMATRIX &candidate, const char *formula) {
    if (*hasWorld || !IsWorldCandidateForDecomposition(candidate)) {
      return false;
}
    *world = candidate;
    *hasWorld = true;
    g_combinedDecompDebug.solvedWorld = true;
    snprintf(g_combinedDecompDebug.worldFormula,
             sizeof(g_combinedDecompDebug.worldFormula), "%s", formula);
    return true;
  };

  auto solveView = [&](const D3DMATRIX &candidate, const char *formula) {
    if (*hasView || !IsViewCandidateForDecomposition(candidate)) {
      return false;
}
    *view = candidate;
    *hasView = true;
    g_combinedDecompDebug.solvedView = true;
    snprintf(g_combinedDecompDebug.viewFormula,
             sizeof(g_combinedDecompDebug.viewFormula), "%s", formula);
    return true;
  };

  auto solveProjection = [&](const D3DMATRIX &candidate, const char *formula) {
    if (*hasProjection || !IsProjectionCandidateForDecomposition(candidate)) {
      return false;
}
    *projection = candidate;
    *hasProjection = true;
    g_combinedDecompDebug.solvedProjection = true;
    snprintf(g_combinedDecompDebug.projectionFormula,
             sizeof(g_combinedDecompDebug.projectionFormula), "%s", formula);
    return true;
  };
  ...
  for (int pass = 0; pass < 8 && changed; ++pass) {
    ...
    if (knownHasVp && !*hasProjection && !*hasView && hasGeneratedProjection &&
        g_config.allowGeneratedProjectionForVPDecomposition) {
      D3DMATRIX solvedView = {};
      if (TrySolveFromInverseLeftMultiply(*generatedProjection, knownVp,
                                          &solvedView) &&
        ...
        *hasView = true;
        ...
        g_combinedDecompDebug.solvedView = true;
        snprintf(g_combinedDecompDebug.projectionFormula,
                 sizeof(g_combinedDecompDebug.projectionFormula),
                 "Projection = Generated manual/auto matrix (config-enabled)");
        snprintf(g_combinedDecompDebug.viewFormula,
                 sizeof(g_combinedDecompDebug.viewFormula),
  ...
  if (g_config.combinedDecompositionLog && g_combinedDecompDebug.attempted) {
    LogMsg("Combined decomposition result: world=%s view=%s projection=%s.",
           *hasWorld ? "yes" : "no", *hasView ? "yes" : "no",
...
void ExtractCameraFromMVP(const D3DMATRIX &mvp, D3DMATRIX *viewOut) {
  ...
  float r0len =
      sqrtf(mvp._11 * mvp._11 + mvp._12 * mvp._12 + mvp._13 * mvp._13);
  float r1len =
      sqrtf(mvp._21 * mvp._21 + mvp._22 * mvp._22 + mvp._23 * mvp._23);
  float r2len =
      sqrtf(mvp._31 * mvp._31 + mvp._32 * mvp._32 + mvp._33 * mvp._33);

  if (r0len > 0.001F && r1len > 0.001F && r2len > 0.001F) {
    // Normalize to get rotation
    viewOut->_11 = mvp._11 / r0len;
    viewOut->_12 = mvp._12 / r0len;
    viewOut->_13 = mvp._13 / r0len;
    viewOut->_21 = mvp._21 / r1len;
    viewOut->_22 = mvp._22 / r1len;
    viewOut->_23 = mvp._23 / r1len;
    viewOut->_31 = mvp._31 / r2len;
    viewOut->_32 = mvp._32 / r2len;
    viewOut->_33 = mvp._33 / r2len;
...
auto LooksLikeView(const D3DMATRIX &m) -> bool {
  ...
  if (fabsf(row0len - 1.0F) > 0.1F) {
    return false;
}
  if (fabsf(row1len - 1.0F) > 0.1F) {
    return false;
}
  if (fabsf(row2len - 1.0F) > 0.1F) {
    return false;
}
...
public:
  ...
  [[nodiscard]] auto GetReal() const -> IDirect3DPixelShader9 * { return m_real; }
  [[nodiscard]] auto GetKey() const -> uintptr_t { return m_key; }
  ...
  HRESULT STDMETHODCALLTYPE GetFunction(void *pData,
    ...
    return m_real->GetFunction(pData, pSizeOfData);
  }
};
...
public:
  ...
  [[nodiscard]] auto GetReal() const -> IDirect3DVertexShader9 * { return m_real; }
  [[nodiscard]] auto GetKey() const -> uintptr_t { return m_key; }
...
public:
  WrappedD3D9Device(IDirect3DDevice9 *real) : m_real(real) {
    if (m_real != nullptr) {
      m_real->QueryInterface(__uuidof(IDirect3DDevice9Ex),
                             reinterpret_cast<void **>(&m_realEx));
    }
  ...
  void EmitFixedFunctionTransforms() {
    ...
    if (g_forceFfpTransform) {
      auto shaderKey = reinterpret_cast<uintptr_t>(m_currentVertexShader);
      auto it = g_shaderRecords.find(shaderKey);
      if (it != g_shaderRecords.end() && it->second.isFFPTransform) {
        const int base = g_manualTransformBaseOverride >= 0
                             ? g_manualTransformBaseOverride
                             : it->second.transformConstantBase;
        ShaderConstantState *state = GetShaderState(shaderKey, false);
        D3DMATRIX m = {};
        if ((state != nullptr) && TryBuildMatrixSnapshot(*state, base, 4, false, &m)) {
          m_currentWorld = m;
          m_hasWorld = true;
        }
      }
    }
    // Keep strict game profiles deterministic even if generic SetTransform
    // compatibility is enabled.
    if (g_activeGameProfile == GameProfile_None &&
        g_config.setTransformBypassProxyWhenGameProvides &&
        g_gameSetTransformAnySeen) {
      return;
    }
    if (g_activeGameProfile == GameProfile_MetalGearRising) {
      // MGR profile is strict: only emit transforms when all three known
      // registers have been captured. Never emit identity/fallback transforms
      // in this mode.
      if (!(m_hasWorld && m_hasView && m_hasProj)) {
        snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                 "MGR draw skipped: missing matrix/matrices (Proj=%s View=%s "
                 "World=%s).",
    ...
    if (g_activeGameProfile == GameProfile_DevilMayCry4) {
      if (!(m_hasWorld && m_hasView && m_hasProj)) {
        snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                 "DMC4 draw skipped: missing matrix/matrices (World=%s View=%s "
                 "Proj=%s).",
                 m_hasWorld ? "ready" : "missing",
                 m_hasView ? "ready" : "missing",
                 m_hasProj ? "ready" : "missing");
    ...
    if (g_activeGameProfile == GameProfile_Barnyard) {
      const bool useGameViewProj =
          g_config.barnyardUseGameSetTransformsForViewProjection;
      if (!m_hasWorld || (useGameViewProj && (!m_hasView || !m_hasProj))) {
        snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                 "Barnyard draw skipped: missing matrices (World=%s View=%s "
                 "Proj=%s).",
                 m_hasWorld ? "ready" : "missing",
                 m_hasView ? "ready" : "missing",
                 m_hasProj ? "ready" : "missing");
      ...
      // Keep Barnyard profile semantics deterministic: WORLD comes from shader
      // constants.
      m_real->SetTransform(D3DTS_WORLD, &m_currentWorld);
      if (useGameViewProj) {
        m_real->SetTransform(D3DTS_VIEW, &m_currentView);
    ...
    const bool strictMgrProfileActive =
        (g_activeGameProfile == GameProfile_MetalGearRising);

    if (g_config.experimentalCustomProjectionEnabled &&
        !strictMgrProfileActive) {
      const bool projectionMissing = !m_hasProj;
      const bool projectionOverrideAllowed =
      ...
      const bool mvpBlocksProjection =
          hasMvp && !g_config.experimentalCustomProjectionOverrideCombinedMVP;
      shouldApplyCustomProjection =
          (projectionMissing || projectionOverrideAllowed) &&
          !mvpBlocksProjection;
    }
    ...
    if (shouldApplyCustomProjection) {
      ...
      UINT width = 0;
      UINT height = 0;
      if (BuildExperimentalCustomProjectionMatrix(
              m_real, m_hwnd, &customProjection, &usedAuto, &resolvedAspect,
              &width, &height)) {
        ...
        if (usedAuto) {
          snprintf(g_customProjectionStatus, sizeof(g_customProjectionStatus),
                   "Experimental projection active (auto): %ux%u aspect=%.4f "
                   "fov=%.2f near=%.4f far=%.2f.",
    ...
    if (m_worldLastFrame >= 0 && g_frameCount > m_worldLastFrame + 1) {
      LogMsg("World matrix stale (last update frame %d, current %d); emitting "
             "identity.",
             m_worldLastFrame, g_frameCount);
      emitWorld = identity;
    }
    if (m_viewLastFrame >= 0 && g_frameCount > m_viewLastFrame + 1) {
      LogMsg("View matrix stale (last update frame %d, current %d); emitting "
             "identity.",
             m_viewLastFrame, g_frameCount);
      emitView = identity;
    }
    if (m_projLastFrame >= 0 && g_frameCount > m_projLastFrame + 1) {
      LogMsg("Projection matrix stale (last update frame %d, current %d); "
             "emitting identity.",
             m_projLastFrame, g_frameCount);
      emitProj = identity;
    }
    ...
    if (g_config.experimentalInverseViewAsWorld && m_hasView &&
      ...
      if (viewLooksValid ||
        ...
        if (TryBuildWorldFromView(
                emitView, g_config.experimentalInverseViewAsWorldFast,
          ...
          if (!fastEligible && g_config.experimentalInverseViewAsWorldFast) {
            LogMsg("Fast inverse requested but view matrix did not qualify "
                   "(possible scaling/shear); used full inverse.");
          }
        }
      }
    }
  ...
  HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid,
    ...
    if (riid == __uuidof(IDirect3DDevice9Ex) && (m_realEx != nullptr)) {
  ...
  SetVertexShaderConstantF(UINT StartRegister, const float *pConstantData,
                           UINT Vector4fCount) override {
    auto shaderKey = reinterpret_cast<uintptr_t>(m_currentVertexShader);
    if (g_constantUploadRecordingEnabled) {
      RecordConstantUpload(ConstantUploadStage_Vertex, shaderKey, StartRegister,
                           Vector4fCount);
    }
    auto recIt = g_shaderRecords.find(shaderKey);
    if (recIt != g_shaderRecords.end()) {
    ...
    if (!profileIsMgr && !profileIsBarnyard &&
        BuildOverriddenConstants(*state, StartRegister, Vector4fCount,
                                 pConstantData, overrideScratch)) {
    ...
    for (UINT i = 0; i < Vector4fCount; i++) {
      ...
      memcpy(state->constants[reg], effectiveConstantData + i * 4,
             sizeof(state->constants[reg]));
      state->valid[reg] = true;
      UpdateVariance(*state, static_cast<int>(reg),
                     effectiveConstantData + i * 4);

      GlobalVertexRegisterState &globalState = g_allVertexRegisters[reg];
      memcpy(globalState.value, effectiveConstantData + i * 4,
             sizeof(globalState.value));
    ...
    if (!profileIsMgr && !profileIsBarnyard && shaderKey != 0) {
      for (int slot = 0; slot < MatrixSlot_Count; slot++) {
        ...
        D3DMATRIX manualMat = {};
        if (!TryBuildMatrixSnapshot(*state, binding.baseRegister, binding.rows,
                                    false, &manualMat)) {
          continue;
        }
        ...
        if (slot == MatrixSlot_World) {
          ...
          m_hasWorld = true;
          m_worldLastFrame = g_frameCount;
          StoreWorldMatrix(m_currentWorld, shaderKey, binding.baseRegister,
                           binding.rows, false, true);
        } else if (slot == MatrixSlot_View) {
          m_currentView = manualMat;
          m_hasView = true;
          m_viewLastFrame = g_frameCount;
          StoreViewMatrix(m_currentView, shaderKey, binding.baseRegister,
                          binding.rows, false, true);
        } else if (slot == MatrixSlot_Projection) {
          ...
          StoreProjectionMatrix(m_currentProj, shaderKey, binding.baseRegister,
                                binding.rows, false, true);
        } else if (slot == MatrixSlot_MVP) {
          StoreMVPMatrix(manualMat, shaderKey, binding.baseRegister,
                         binding.rows, false, true);
        } else if (slot == MatrixSlot_VP) {
          StoreVPMatrix(manualMat, shaderKey, binding.baseRegister,
                        binding.rows, false, true);
        } else if (slot == MatrixSlot_WV) {
          StoreWVMatrix(manualMat, shaderKey, binding.baseRegister,
                        binding.rows, false, true);
        }
      }
    }
    ...
    if (profileIsMgr) {
      ...
      // - persist projection/view across draws until overwritten by the same
      // known registers
      auto tryExtractMgrMatrix = [&](int baseRegister,
                                     D3DMATRIX *outMat) -> bool {
        if ((outMat == nullptr) || (effectiveConstantData == nullptr) || baseRegister < 0) {
          return false;
        }
        const UINT uploadStart = StartRegister;
        const UINT uploadEnd = (Vector4fCount == 0)
                                   ? StartRegister
                                   : (StartRegister + Vector4fCount - 1);
        const UINT matrixEnd = static_cast<UINT>(baseRegister + 3);
        if (uploadStart > static_cast<UINT>(baseRegister) ||
            uploadEnd < matrixEnd) {
          return false;
        }
      ...
      if (tryExtractMgrMatrix(4, &mat)) {
        ...
        if (g_mgrProjectionRegisterValid) {
          ...
          StoreProjectionMatrix(
              m_currentProj, shaderKey, 4, 4, false, true,
              "MetalGearRising profile projection (c4-c7, non-typical)");
          snprintf(
              g_profileStatusMessage, sizeof(g_profileStatusMessage),
      ...
      if (tryExtractMgrMatrix(8, &mat)) {
        ...
        if (!g_mgrProjectionRegisterValid && m_mgrrUseAutoProjection) {
          ...
          if (TryExtractProjectionFromCombined(mat, nullptr, false,
            ...
            StoreProjectionMatrix(
                m_currentProj, shaderKey, 8, 4, false, true,
        ...
        if (haveProjectionForViewDerivation) {
          ...
          if (InvertMatrix4x4Deterministic(resolvedProjection, &projectionInv,
                                           nullptr)) {
          ...
          g_profileViewDerivedFromInverse = false;
          snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                   "MGR VP detected but projection inversion failed. Enable "
                   "auto projection to prefer generated projection.");
        }
      }
      ...
      return m_real->SetVertexShaderConstantF(
          StartRegister, effectiveConstantData, Vector4fCount);
    }
    ...
    if (profileIsBarnyard) {
      ...
      if (g_barnyardForceWorldFromC0 &&
          TryBuildMatrixFromConstantUpdate(effectiveConstantData, StartRegister,
                                           Vector4fCount, 0, 4, false, &mat)) {
        m_currentWorld = mat;
        m_hasWorld = true;
        m_worldLastFrame = g_frameCount;
        worldCaptured = true;
      ...
      if (!worldCaptured && g_config.worldMatrixRegister >= 0 &&
          TryBuildMatrixFromConstantUpdate(
              effectiveConstantData, StartRegister, Vector4fCount,
        ...
        worldCaptured = true;
        g_profileCoreRegistersSeen[2] = true;
        StoreWorldMatrix(m_currentWorld, shaderKey,
                         g_config.worldMatrixRegister, 4, false, true,
                         "Barnyard profile world (explicit register override)");
      }

      if (!worldCaptured && g_config.autoDetectMatrices &&
          (effectiveConstantData != nullptr) && Vector4fCount >= 3) {
        for (UINT rows : {4U, 3U}) {
          ...
          for (UINT offset = 0; offset + rows <= Vector4fCount; ++offset) {
            ...
            D3DMATRIX candidate = {};
            if (!TryBuildMatrixFromConstantUpdate(
                    effectiveConstantData + offset * 4, baseReg, rows,
                    static_cast<int>(baseReg), static_cast<int>(rows), false,
                    &candidate)) {
              continue;
            }

            MatrixClassification cls = ClassifyMatrixDeterministic(
                candidate, static_cast<int>(rows), Vector4fCount, StartRegister,
                baseReg);
            bool transposed = false;
            if (cls == MatrixClass_None && g_probeTransposedLayouts) {
              D3DMATRIX t = TransposeMatrix(candidate);
              cls = ClassifyMatrixDeterministic(t, static_cast<int>(rows),
                                                Vector4fCount, StartRegister,
                                                baseReg);
      ...
      if (worldCaptured) {
        snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                 "Barnyard profile active: forwarding WORLD only; "
                 "VIEW/PROJECTION SetTransform blocked.");
      } else {
        snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                 "Barnyard profile active: waiting for world matrix in shader "
                 "constants.");
      }
    ...
    const bool profileActive =
        profileIsMgr || profileIsDmc4 || profileIsBarnyard;

    auto tryExtractProfileMatrix = [&](int baseRegister,
                                       D3DMATRIX *outMat) -> bool {
      if ((outMat == nullptr) || (effectiveConstantData == nullptr) || baseRegister < 0) {
        return false;
      }
    ...
    if (profileIsDmc4) {
      ...
      if (tryExtractProfileMatrix(g_profileLayout.combinedMvpBase, &mat)) {
        anyCaptured = true;
        StoreMVPMatrix(mat, shaderKey, g_profileLayout.combinedMvpBase, 4,
                       false, true,
        ...
        g_profileCoreRegistersSeen[0] = true;
        StoreWorldMatrix(m_currentWorld, shaderKey, g_profileLayout.worldBase,
                         4, false, true, "DevilMayCry4 profile world (c0-c3)");
      }
      ...
      if (tryExtractProfileMatrix(g_profileLayout.viewInverseBase, &mat)) {
        ...
        g_profileCoreRegistersSeen[1] = true;
        g_profileViewDerivedFromInverse = false;
        StoreViewMatrix(m_currentView, shaderKey,
                        g_profileLayout.viewInverseBase, 4, false, true,
      ...
      if (tryExtractProfileMatrix(g_profileLayout.projectionBase, &mat)) {
        ...
        g_projectionDetectedFovRadians = ExtractFOV(mat);
        g_profileCoreRegistersSeen[2] = true;
        StoreProjectionMatrix(m_currentProj, shaderKey,
                              g_profileLayout.projectionBase, 4, false, true,
                              "DevilMayCry4 profile projection (c8-c11)");
      }

      if (anyCaptured) {
        snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                 "DMC4 profile active: strict mapping MVP/World=c0-c3 "
                 "View=c4-c7 Projection=c8-c11.");
      } else {
        snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
                 "DMC4 profile active but upload did not hit c0-c11 transform "
                 "registers.");
      }
    ...
    auto tryExplicitRegisterOverride = [&](MatrixSlot slot,
                                           int configuredRegister) {
      if (profileActive) {
        return;
      }
      if (configuredRegister < 0 || (effectiveConstantData == nullptr)) {
        return;
      }
      ...
      for (UINT rows : {4U, 3U}) {
        ...
        if (slot == MatrixSlot_World) {
          ...
          m_hasWorld = true;
          m_worldLastFrame = g_frameCount;
          StoreWorldMatrix(m_currentWorld, shaderKey, configuredRegister,
                           static_cast<int>(rows), false, true,
                           "explicit register override");
        } else if (slot == MatrixSlot_View) {
          m_currentView = mat;
          m_hasView = true;
          m_viewLastFrame = g_frameCount;
          StoreViewMatrix(m_currentView, shaderKey, configuredRegister,
                          static_cast<int>(rows), false, true,
                          "explicit register override");
        } else if (slot == MatrixSlot_Projection) {
          m_currentProj = mat;
          ...
          g_projectionDetectedHandedness = ProjectionHandedness_Unknown;
          g_projectionDetectedFovRadians = 0.0F;
          StoreProjectionMatrix(m_currentProj, shaderKey, configuredRegister,
                                static_cast<int>(rows), false, true,
                                "explicit register override");
        } else if (slot == MatrixSlot_MVP) {
          StoreMVPMatrix(mat, shaderKey, configuredRegister,
                         static_cast<int>(rows), false, true,
                         "explicit register override");
        } else if (slot == MatrixSlot_VP) {
          StoreVPMatrix(mat, shaderKey, configuredRegister,
                        static_cast<int>(rows), false, true,
                        "explicit register override");
        } else if (slot == MatrixSlot_WV) {
          StoreWVMatrix(mat, shaderKey, configuredRegister,
                        static_cast<int>(rows), false, true,
                        "explicit register override");
        }
        return;
    ...
    tryExplicitRegisterOverride(MatrixSlot_Projection,
    ...
    auto updateFromClassification = [&](D3DMATRIX mat, UINT baseReg, int rows,
                                        bool transposed) {
      MatrixClassification cls = ClassifyMatrixDeterministic(
          mat, rows, Vector4fCount, StartRegister, baseReg);

      if (cls == MatrixClass_Projection && g_config.projMatrixRegister < 0 &&
        ...
        if (projectionInfo.fovRadians < g_config.minFOV ||
            projectionInfo.fovRadians > g_config.maxFOV) {
          return;
}
        ...
        if (sameSource) {
          ...
          StoreProjectionMatrix(m_currentProj, shaderKey,
                                static_cast<int>(baseReg), rows, transposed,
                                false, "deterministic structural projection");
          LogMsg("Projection accepted: c%d-c%d rows=%d fov=%.2f deg (%s)",
                 static_cast<int>(baseReg),
                 static_cast<int>(baseReg) + rows - 1, rows,
                 projectionInfo.fovRadians * 180.0F / 3.14159265F,
                 ProjectionHandednessLabel(projectionInfo.handedness));
        }
      } else if (cls == MatrixClass_View && !suppressViewFromUpload &&
        ...
        if (sameSource) {
          if (m_hasProj && m_projDetectedFrame == g_frameCount &&
            ...
            // This is WV, VP, or some other combined form — not a pure view
            // matrix. Do not mark slotResolvedStructurally so detection can
            // continue.
            return;
          }
      ...
      } else if (cls == MatrixClass_World && !suppressWorldFromUpload &&
        ...
        StoreWorldMatrix(m_currentWorld, shaderKey, static_cast<int>(baseReg),
                         rows, transposed, false,
                         "deterministic structural world");
                 ...
                 rows == 4) {
        StoreMVPMatrix(
            mat, shaderKey, static_cast<int>(baseReg), rows, transposed, false,
    ...
    if (allowStructuralDetection && (effectiveConstantData != nullptr) &&
        Vector4fCount >= 12) {
    ...
    if (allowStructuralDetection && (effectiveConstantData != nullptr) &&
      ...
      for (UINT rows : {4U, 3U}) {
        ...
        for (UINT offset = 0; offset + rows <= Vector4fCount; ++offset) {
          ...
          MatrixClassification directClass = ClassifyMatrixDeterministic(
              mat, static_cast<int>(rows), Vector4fCount, StartRegister,
              baseReg);
          bool transposed = false;
          if (directClass == MatrixClass_None && g_probeTransposedLayouts) {
            D3DMATRIX t = TransposeMatrix(mat);
            MatrixClassification transposedClass = ClassifyMatrixDeterministic(
                t, static_cast<int>(rows), Vector4fCount, StartRegister,
                baseReg);
          ...
          MatrixClassification finalClass = ClassifyMatrixDeterministic(
              mat, static_cast<int>(rows), Vector4fCount, StartRegister,
              baseReg);
          if (rows == 3U &&
              (finalClass == MatrixClass_View ||
               finalClass == MatrixClass_World) &&
              IsThreeRowPrefixOfPerspectiveMatrix(effectiveConstantData,
                                                  StartRegister, Vector4fCount,
                                                  baseReg, transposed)) {
            finalClass = MatrixClass_None;
          }
          if (finalClass == MatrixClass_None && g_probeInverseView &&
              rows == 4U) {
            const float row0Len = sqrtf(
                Dot3(mat._11, mat._12, mat._13, mat._11, mat._12, mat._13));
            const float row1Len = sqrtf(
                Dot3(mat._21, mat._22, mat._23, mat._21, mat._22, mat._23));
            const float row2Len = sqrtf(
                Dot3(mat._31, mat._32, mat._33, mat._31, mat._32, mat._33));
            const bool orthonormal =
                fabsf(row0Len - 1.0F) < 0.05F &&
                fabsf(row1Len - 1.0F) < 0.05F &&
                fabsf(row2Len - 1.0F) < 0.05F &&
                fabsf(Dot3(mat._11, mat._12, mat._13, mat._21, mat._22,
                           mat._23)) < 0.05F &&
                fabsf(Dot3(mat._11, mat._12, mat._13, mat._31, mat._32,
                           mat._33)) < 0.05F &&
                fabsf(Dot3(mat._21, mat._22, mat._23, mat._31, mat._32,
                           mat._33)) < 0.05F;
            if (orthonormal) {
              D3DMATRIX inverseView = InvertSimpleRigidView(mat);
              MatrixClassification inverseClass = ClassifyMatrixDeterministic(
                  inverseView, static_cast<int>(rows), Vector4fCount,
                  StartRegister, baseReg);
  ...
  done_scanning:
    ...
    if (g_config.logAllConstants && m_constantLogThrottle == 0 &&
        Vector4fCount >= 4) {
      LogMsg("SetVertexShaderConstantF: c%d-%d (%d vectors)", StartRegister,
             StartRegister + Vector4fCount - 1, Vector4fCount);
      for (UINT i = 0; i < Vector4fCount && i < 4; i++) {
        LogMsg(
            "  c%d: [%.3f, %.3f, %.3f, %.3f]", StartRegister + i,
    ...
    if (g_config.allowGeneratedProjectionForVPDecomposition) {
      hasGeneratedProjection = BuildExperimentalCustomProjectionMatrix(
          m_real, m_hwnd, &generatedProjection, nullptr, nullptr, nullptr,
          nullptr);
    }

    TryDecomposeCombinedMatricesDeterministic(
        &m_currentWorld, &m_hasWorld, &m_currentView, &m_hasView,
        &m_currentProj, &m_hasProj, &combinedMvp, hasMvp, &combinedMv, hasMv,
        &combinedVp, hasVp, &generatedProjection, hasGeneratedProjection);
  ...
  HRESULT STDMETHODCALLTYPE Present(const RECT *pSourceRect,
    ...
    // Reset per-frame source locks. Locks set during SetVertexShaderConstantF
    // calls this frame will be validated against these — after Present they
    // reset for the next frame. This ensures correct behavior regardless of
    // BeginScene call count.
    if (g_activeGameProfile == GameProfile_None) {
      m_viewLockedShader = 0;
    ...
    if (g_config.enableMemoryScanner && g_config.memoryScannerIntervalSec > 0) {
      ...
      if (g_memoryScannerLastTick == 0 ||
          nowTick - g_memoryScannerLastTick >=
              static_cast<DWORD>(g_config.memoryScannerIntervalSec) * 1000U) {
        StartMemoryScanner();
    ...
    if (g_frameCount % 300 == 0) {
      LogMsg("Frame %d - hasView: %d, hasProj: %d", g_frameCount, static_cast<int>(m_hasView),
             static_cast<int>(m_hasProj));
    }
    ...
    if (g_requestManualEmit) {
      ...
      if (g_activeGameProfile == GameProfile_Barnyard) {
        snprintf(
            g_manualEmitStatus, sizeof(g_manualEmitStatus),
            g_config.barnyardUseGameSetTransformsForViewProjection
                ? "Sent cached World/View/Projection matrices to RTX Remix via "
                  "SetTransform()."
    ...
    return m_real->Present(pSourceRect, pDestRect, hDestWindowOverride,
                           pDirtyRegion);
  }
  ...
  CreateAdditionalSwapChain(D3DPRESENT_PARAMETERS *pPresentationParameters,
    ...
    return m_real->CreateAdditionalSwapChain(pPresentationParameters,
                                             pSwapChain);
  }
  HRESULT STDMETHODCALLTYPE
  GetSwapChain(UINT iSwapChain, IDirect3DSwapChain9 **pSwapChain) override {
  ...
  HRESULT STDMETHODCALLTYPE CreateTexture(UINT Width, UINT Height, UINT Levels,
    ...
    return m_real->CreateTexture(Width, Height, Levels, Usage, Format, Pool,
  ...
  HRESULT STDMETHODCALLTYPE CreateVolumeTexture(
      ...
      HANDLE *pSharedHandle) override {
    return m_real->CreateVolumeTexture(Width, Height, Depth, Levels, Usage,
  ...
  HRESULT STDMETHODCALLTYPE CreateCubeTexture(
    ...
    return m_real->CreateCubeTexture(EdgeLength, Levels, Usage, Format, Pool,
  ...
  HRESULT STDMETHODCALLTYPE CreateVertexBuffer(
    ...
    return m_real->CreateVertexBuffer(Length, Usage, FVF, Pool, ppVertexBuffer,
                                      pSharedHandle);
  }
  HRESULT STDMETHODCALLTYPE CreateIndexBuffer(
      UINT Length, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
      IDirect3DIndexBuffer9 **ppIndexBuffer, HANDLE *pSharedHandle) override {
    return m_real->CreateIndexBuffer(Length, Usage, Format, Pool, ppIndexBuffer,
                                     pSharedHandle);
  }
  HRESULT STDMETHODCALLTYPE CreateRenderTarget(
    ...
    return m_real->CreateRenderTarget(Width, Height, Format, MultiSample,
  ...
  HRESULT STDMETHODCALLTYPE CreateDepthStencilSurface(
      ...
      IDirect3DSurface9 **ppSurface, HANDLE *pSharedHandle) override {
    return m_real->CreateDepthStencilSurface(Width, Height, Format, MultiSample,
  ...
  UpdateSurface(IDirect3DSurface9 *pSourceSurface, const RECT *pSourceRect,
    ...
    return m_real->UpdateSurface(pSourceSurface, pSourceRect,
  ...
  HRESULT STDMETHODCALLTYPE
  GetRenderTargetData(IDirect3DSurface9 *pRenderTarget,
  ...
  HRESULT STDMETHODCALLTYPE StretchRect(IDirect3DSurface9 *pSourceSurface,
    ...
    return m_real->StretchRect(pSourceSurface, pSourceRect, pDestSurface,
                               pDestRect, Filter);
  }
  ...
  HRESULT STDMETHODCALLTYPE CreateOffscreenPlainSurface(
      ...
      IDirect3DSurface9 **ppSurface, HANDLE *pSharedHandle) override {
    return m_real->CreateOffscreenPlainSurface(Width, Height, Format, Pool,
                                               ppSurface, pSharedHandle);
  }
  ...
  HRESULT STDMETHODCALLTYPE BeginScene() override {
    g_combinedDecompDebug = {};
    if (g_activeGameProfile == GameProfile_MetalGearRising) {
      // MGR frame lifecycle: keep projection/view persistent across draws and
      // frames, but require a fresh world upload for each frame.
      CreateIdentityMatrix(&m_currentWorld);
      m_hasWorld = false;
  ...
  HRESULT STDMETHODCALLTYPE SetTransform(D3DTRANSFORMSTATETYPE State,
    ...
    if (transformIdx >= 0 && (pMatrix != nullptr)) {
      g_gameSetTransformSeen[transformIdx] = true;
      g_gameSetTransformAnySeen = true;

      const bool allowGenericSetTransformCompatibility =
          (g_activeGameProfile == GameProfile_None);
      if (allowGenericSetTransformCompatibility &&
          g_config.setTransformBypassProxyWhenGameProvides) {
        if (State == D3DTS_WORLD) {
          ...
          m_projLastFrame = g_frameCount;
          StoreProjectionMatrix(
              m_currentProj, 0, -1, 4, false, true,
      ...
      if (allowGenericSetTransformCompatibility &&
        ...
        if (State == D3DTS_WORLD) {
          ...
          m_worldLastFrame = g_frameCount;
          StoreWorldMatrix(
              m_currentWorld, 0, -1, 4, false, true,
    ...
    if (g_activeGameProfile == GameProfile_Barnyard &&
      ...
      if (!g_config.barnyardUseGameSetTransformsForViewProjection || (pMatrix == nullptr)) {
        return D3D_OK;
      }
      ...
      if (State == D3DTS_VIEW) {
        ...
        g_profileCoreRegistersSeen[0] = true;
        StoreViewMatrix(
            m_currentView, 0, -1, 4, false, true,
            SUCCEEDED(getHr)
        ...
        StoreProjectionMatrix(
            m_currentProj, 0, -1, 4, false, true,
            SUCCEEDED(getHr)
                ? "Barnyard intercepted game "
                  "SetTransform(Projection)+GetTransform"
                : "Barnyard intercepted game SetTransform(Projection)");
      }

      snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
               "Barnyard intercepted game %s transform and cached for "
               "draw-time forwarding.",
  ...
  HRESULT STDMETHODCALLTYPE MultiplyTransform(
      D3DTRANSFORMSTATETYPE State, const D3DMATRIX *pMatrix) override {
    return m_real->MultiplyTransform(State, pMatrix);
  }
  HRESULT STDMETHODCALLTYPE
  SetViewport(const D3DVIEWPORT9 *pViewport) override {
  ...
  HRESULT STDMETHODCALLTYPE
  SetMaterial(const D3DMATERIAL9 *pMaterial) override {
  ...
  HRESULT STDMETHODCALLTYPE
  GetClipStatus(D3DCLIPSTATUS9 *pClipStatus) override {
  ...
  HRESULT STDMETHODCALLTYPE DrawPrimitive(D3DPRIMITIVETYPE PrimitiveType,
    ...
    if ((g_pauseRendering ||
         IsCurrentShaderDrawDisabled(m_currentVertexShader)) &&
        !g_isRenderingImGui) {
      return D3D_OK;
    }
    EmitFixedFunctionTransforms();
    if (g_shaderRecords.count(g_activeVertexShaderKey) != 0u) {
      g_shaderRecords[g_activeVertexShaderKey].usageCount++;
}
    if (g_shaderRecords.count(g_activePixelShaderKey) != 0u) {
      g_shaderRecords[g_activePixelShaderKey].usageCount++;
}
  ...
  HRESULT STDMETHODCALLTYPE DrawIndexedPrimitive(
    ...
    return m_real->DrawIndexedPrimitive(PrimitiveType, BaseVertexIndex,
  ...
  HRESULT STDMETHODCALLTYPE DrawPrimitiveUP(
    ...
    EmitFixedFunctionTransforms();
    return m_real->DrawPrimitiveUP(PrimitiveType, PrimitiveCount,
  ...
  HRESULT STDMETHODCALLTYPE DrawIndexedPrimitiveUP(
    ...
    EmitFixedFunctionTransforms();
    return m_real->DrawIndexedPrimitiveUP(
  ...
  HRESULT STDMETHODCALLTYPE ProcessVertices(
    ...
    return m_real->ProcessVertices(SrcStartIndex, DestIndex, VertexCount,
      ...
      if (SUCCEEDED(pDecl->GetDeclaration(elems, &num))) {
        for (UINT i = 0; i < num; ++i) {
          ...
          if (e.Usage == D3DDECLUSAGE_POSITION) {
            g_activeVertexDeclInfo.hasPosition = true;
}
          if (e.Usage == D3DDECLUSAGE_NORMAL) {
            g_activeVertexDeclInfo.hasNormal = true;
}
          if (e.Usage == D3DDECLUSAGE_TANGENT) {
            g_activeVertexDeclInfo.hasTangent = true;
}
          if (e.Usage == D3DDECLUSAGE_BINORMAL) {
            g_activeVertexDeclInfo.hasBinormal = true;
}
          if (e.Usage == D3DDECLUSAGE_TEXCOORD) {
            g_activeVertexDeclInfo.texcoordCount =
                (std::max)(g_activeVertexDeclInfo.texcoordCount,
                           static_cast<int>(e.UsageIndex) + 1);
}
        }
      }
    }
    ...
    *ppShader = new WrappedVertexShader9(realShader);
    ...
    if (pFunction != nullptr) {
      for (size_t i = 0; i < 16384; ++i) {
        ...
        if ((pFunction[i] & D3DSI_OPCODE_MASK) == D3DSIO_END) {
          break;
}
      }
    }
    RegisterShaderBytecode(
        reinterpret_cast<WrappedVertexShader9 *>(*ppShader)->GetKey(),
        ShaderStage_Vertex, data);
    return hr;
  }
  HRESULT STDMETHODCALLTYPE
  SetVertexShader(IDirect3DVertexShader9 *pShader) override {
    ...
    if (pShader != nullptr) {
      auto *wrapped =
          static_cast<WrappedVertexShader9 *>(pShader);
    ...
    if ((pShader != nullptr) && (g_shaderRecords.count(g_activeShaderKey) != 0u) &&
        g_shaderRecords[g_activeShaderKey].replacementEnabled &&
        (g_shaderRecords[g_activeShaderKey].replacementShader != nullptr)) {
      realShader = reinterpret_cast<IDirect3DVertexShader9 *>(
          g_shaderRecords[g_activeShaderKey].replacementShader);
    }
  ...
  HRESULT STDMETHODCALLTYPE GetVertexShaderConstantF(
    ...
    return m_real->GetVertexShaderConstantF(StartRegister, pConstantData,
                                            Vector4fCount);
  }
  HRESULT STDMETHODCALLTYPE
  SetVertexShaderConstantI(UINT StartRegister, const int *pConstantData,
                           UINT Vector4iCount) override {
    return m_real->SetVertexShaderConstantI(StartRegister, pConstantData,
                                            Vector4iCount);
  }
  HRESULT STDMETHODCALLTYPE GetVertexShaderConstantI(
      UINT StartRegister, int *pConstantData, UINT Vector4iCount) override {
    return m_real->GetVertexShaderConstantI(StartRegister, pConstantData,
                                            Vector4iCount);
  }
  HRESULT STDMETHODCALLTYPE SetVertexShaderConstantB(UINT StartRegister,
                                                     const BOOL *pConstantData,
                                                     UINT BoolCount) override {
    return m_real->SetVertexShaderConstantB(StartRegister, pConstantData,
                                            BoolCount);
  }
  HRESULT STDMETHODCALLTYPE GetVertexShaderConstantB(UINT StartRegister,
                                                     BOOL *pConstantData,
                                                     UINT BoolCount) override {
    return m_real->GetVertexShaderConstantB(StartRegister, pConstantData,
                                            BoolCount);
  }
  HRESULT STDMETHODCALLTYPE SetStreamSource(UINT StreamNumber,
    ...
    return m_real->SetStreamSource(StreamNumber, pStreamData, OffsetInBytes,
                                   Stride);
  }
  HRESULT STDMETHODCALLTYPE
  GetStreamSource(UINT StreamNumber, IDirect3DVertexBuffer9 **ppStreamData,
                  UINT *pOffsetInBytes, UINT *pStride) override {
    return m_real->GetStreamSource(StreamNumber, ppStreamData, pOffsetInBytes,
                                   pStride);
  }
  ...
  HRESULT STDMETHODCALLTYPE CreatePixelShader(
    ...
    *ppShader = new WrappedPixelShader9(realShader);
    ...
    RegisterShaderBytecode(
        reinterpret_cast<WrappedPixelShader9 *>(*ppShader)->GetKey(),
        ShaderStage_Pixel, data);
    return hr;
  }
  HRESULT STDMETHODCALLTYPE
  SetPixelShader(IDirect3DPixelShader9 *pShader) override {
    ...
    if (pShader != nullptr) {
      ...
      if ((g_shaderRecords.count(g_activePixelShaderKey) != 0u) &&
          g_shaderRecords[g_activePixelShaderKey].replacementEnabled &&
          (g_shaderRecords[g_activePixelShaderKey].replacementShader != nullptr)) {
        realShader = reinterpret_cast<IDirect3DPixelShader9 *>(
            g_shaderRecords[g_activePixelShaderKey].replacementShader);
      }
    }
  ...
  HRESULT STDMETHODCALLTYPE
  SetPixelShaderConstantF(UINT StartRegister, const float *pConstantData,
                          UINT Vector4fCount) override {
    auto shaderKey = reinterpret_cast<uintptr_t>(m_currentPixelShader);
    if (g_constantUploadRecordingEnabled) {
      RecordConstantUpload(ConstantUploadStage_Pixel, shaderKey, StartRegister,
                           Vector4fCount);
    }
    return m_real->SetPixelShaderConstantF(StartRegister, pConstantData,
                                           Vector4fCount);
  }
  HRESULT STDMETHODCALLTYPE GetPixelShaderConstantF(
      UINT StartRegister, float *pConstantData, UINT Vector4fCount) override {
    return m_real->GetPixelShaderConstantF(StartRegister, pConstantData,
                                           Vector4fCount);
  }
  HRESULT STDMETHODCALLTYPE
  SetPixelShaderConstantI(UINT StartRegister, const int *pConstantData,
                          UINT Vector4iCount) override {
    return m_real->SetPixelShaderConstantI(StartRegister, pConstantData,
                                           Vector4iCount);
  }
  HRESULT STDMETHODCALLTYPE GetPixelShaderConstantI(
      UINT StartRegister, int *pConstantData, UINT Vector4iCount) override {
    return m_real->GetPixelShaderConstantI(StartRegister, pConstantData,
                                           Vector4iCount);
  }
  HRESULT STDMETHODCALLTYPE SetPixelShaderConstantB(UINT StartRegister,
                                                    const BOOL *pConstantData,
                                                    UINT BoolCount) override {
    return m_real->SetPixelShaderConstantB(StartRegister, pConstantData,
                                           BoolCount);
  }
  HRESULT STDMETHODCALLTYPE GetPixelShaderConstantB(UINT StartRegister,
                                                    BOOL *pConstantData,
                                                    UINT BoolCount) override {
    return m_real->GetPixelShaderConstantB(StartRegister, pConstantData,
                                           BoolCount);
  }
  HRESULT STDMETHODCALLTYPE
  ...
  HRESULT STDMETHODCALLTYPE SetConvolutionMonoKernel(UINT width, UINT height,
    ...
    return m_realEx != nullptr ? m_realEx->SetConvolutionMonoKernel(width, height, rows,
                                                         columns)
                    : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE ComposeRects(IDirect3DSurface9 *pSrc,
                                         ...
                                         D3DCOMPOSERECTSOP op, int xoffset,
                                         int yoffset) override {
    return m_realEx != nullptr
               ? m_realEx->ComposeRects(pSrc, pDst, pSrcRectDescs, numRects,
                                        pDstRectDescs, op, xoffset, yoffset)
               : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE PresentEx(CONST RECT *pSourceRect,
    ...
    return m_realEx != nullptr
               ? m_realEx->PresentEx(pSourceRect, pDestRect,
                                     hDestWindowOverride, pDirtyRegion, dwFlags)
               : Present(pSourceRect, pDestRect, hDestWindowOverride,
                         pDirtyRegion);
  }
  HRESULT STDMETHODCALLTYPE GetGPUThreadPriority(INT *pPriority) override {
    return m_realEx != nullptr ? m_realEx->GetGPUThreadPriority(pPriority)
                    : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE SetGPUThreadPriority(INT Priority) override {
    return m_realEx != nullptr ? m_realEx->SetGPUThreadPriority(Priority)
                    : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE WaitForVBlank(UINT iSwapChain) override {
    return m_realEx != nullptr ? m_realEx->WaitForVBlank(iSwapChain) : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE CheckResourceResidency(
      IDirect3DResource9 **pResourceArray, UINT32 NumResources) override {
    return m_realEx != nullptr
               ? m_realEx->CheckResourceResidency(pResourceArray, NumResources)
               : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE SetMaximumFrameLatency(UINT MaxLatency) override {
    return m_realEx != nullptr ? m_realEx->SetMaximumFrameLatency(MaxLatency)
                    : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE GetMaximumFrameLatency(UINT *pMaxLatency) override {
    return m_realEx != nullptr ? m_realEx->GetMaximumFrameLatency(pMaxLatency)
                    : D3DERR_INVALIDCALL;
  }
  HRESULT STDMETHODCALLTYPE CheckDeviceState(HWND hDestinationWindow) override {
    return m_realEx != nullptr ? m_realEx->CheckDeviceState(hDestinationWindow)
                    : D3DERR_INVALIDCALL;
  }
  ...
  HRESULT STDMETHODCALLTYPE CreateDepthStencilSurfaceEx(
      ...
      DWORD Usage) override {
    return m_realEx != nullptr
               ? m_realEx->CreateDepthStencilSurfaceEx(
                     Width, Height, Format, MultiSample, MultisampleQuality,
  ...
  ResetEx(D3DPRESENT_PARAMETERS *pPresentationParameters,
    ...
    return m_realEx != nullptr ? m_realEx->ResetEx(pPresentationParameters,
                                        pFullscreenDisplayMode)
                    : Reset(pPresentationParameters);
  }
  HRESULT STDMETHODCALLTYPE
  GetDisplayModeEx(UINT iSwapChain, D3DDISPLAYMODEEX *pMode,
                   D3DDISPLAYROTATION *pRotation) override {
    return m_realEx != nullptr ? m_realEx->GetDisplayModeEx(iSwapChain, pMode, pRotation)
                    : D3DERR_INVALIDCALL;
  }
};
...
public:
  ...
  HRESULT STDMETHODCALLTYPE CheckDeviceType(UINT Adapter, D3DDEVTYPE DevType,
    ...
    return m_real->CheckDeviceType(Adapter, DevType, AdapterFormat,
  ...
  HRESULT STDMETHODCALLTYPE CheckDeviceFormat(
    ...
    return m_real->CheckDeviceFormat(Adapter, DeviceType, AdapterFormat, Usage,
                                     RType, CheckFormat);
  }
  ...
  HRESULT STDMETHODCALLTYPE CheckDeviceMultiSampleType(
    ...
    return m_real->CheckDeviceMultiSampleType(Adapter, DeviceType,
  ...
  HRESULT STDMETHODCALLTYPE CheckDepthStencilMatch(
    ...
    return m_real->CheckDepthStencilMatch(Adapter, DeviceType, AdapterFormat,
                                          RenderTargetFormat,
                                          DepthStencilFormat);
  }
  ...
  HRESULT STDMETHODCALLTYPE CheckDeviceFormatConversion(
    ...
    return m_real->CheckDeviceFormatConversion(Adapter, DeviceType,
  ...
  HRESULT STDMETHODCALLTYPE CreateDevice(
    ...
    LogMsg("CreateDevice called - Adapter: %d, DeviceType: %d", Adapter,
           DeviceType);

    IDirect3DDevice9 *realDevice = nullptr;
    HRESULT hr =
        m_real->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags,
                             pPresentationParameters, &realDevice);

    if (SUCCEEDED(hr) && (realDevice != nullptr)) {
...
public:
  ...
  HRESULT STDMETHODCALLTYPE CreateDeviceEx(
    ...
    HRESULT hr = m_real->CreateDeviceEx(Adapter, DeviceType, hFocusWindow,
...
void LoadConfig() {
  ...
  if (!TryReadFileDescription(g_gameExePath, g_gameDisplayName,
                              sizeof(g_gameDisplayName))) {
    snprintf(g_gameDisplayName, sizeof(g_gameDisplayName), "%s",
             g_gameExeName[0] != 0 ? g_gameExeName : "<unknown>");
  }
  ...
  g_config.viewMatrixRegister =
      GetPrivateProfileIntA("CameraProxy", "ViewMatrixRegister", -1, path);
  g_config.projMatrixRegister =
      GetPrivateProfileIntA("CameraProxy", "ProjMatrixRegister", -1, path);
  g_config.worldMatrixRegister =
      GetPrivateProfileIntA("CameraProxy", "WorldMatrixRegister", -1, path);
  g_config.mvpMatrixRegister =
      GetPrivateProfileIntA("CameraProxy", "MVPMatrixRegister", -1, path);
  g_config.mvMatrixRegister =
      GetPrivateProfileIntA("CameraProxy", "MVMatrixRegister", -1, path);
  g_config.vpMatrixRegister =
      GetPrivateProfileIntA("CameraProxy", "VPMatrixRegister", -1, path);
  ...
  g_config.enableLogging =
      GetPrivateProfileIntA("CameraProxy", "EnableLogging", 1, path) != 0;
  g_config.logAllConstants =
      GetPrivateProfileIntA("CameraProxy", "LogAllConstants", 0, path) != 0;
  g_config.autoDetectMatrices =
      GetPrivateProfileIntA("CameraProxy", "AutoDetectMatrices", 0, path) != 0;
  g_config.enableMemoryScanner =
      GetPrivateProfileIntA("CameraProxy", "EnableMemoryScanner", 0, path) != 0;
  g_config.memoryScannerIntervalSec =
      GetPrivateProfileIntA("CameraProxy", "MemoryScannerIntervalSec", 0, path);
  g_config.memoryScannerMaxResults =
      GetPrivateProfileIntA("CameraProxy", "MemoryScannerMaxResults", 25, path);
  GetPrivateProfileStringA("CameraProxy", "MemoryScannerModule", "",
                           g_config.memoryScannerModule, MAX_PATH, path);
  g_config.useRemixRuntime =
      GetPrivateProfileIntA("CameraProxy", "UseRemixRuntime", 1, path) != 0;
  g_config.emitFixedFunctionTransforms =
      GetPrivateProfileIntA("CameraProxy", "EmitFixedFunctionTransforms", 1,
                            path) != 0;
  GetPrivateProfileStringA(
      "CameraProxy", "GameProfile", "", g_config.gameProfile,
  ...
  g_profileCoreRegistersSeen[0] = g_profileCoreRegistersSeen[1] =
      g_profileCoreRegistersSeen[2] = false;
  g_profileOptionalRegistersSeen[0] = g_profileOptionalRegistersSeen[1] = false;
  g_profileViewDerivedFromInverse = false;
  g_profileStatusMessage[0] = '\0';
  g_profileDisableStructuralDetection = false;
  g_barnyardForceWorldFromC0 =
      GetPrivateProfileIntA("CameraProxy", "BarnyardForceWorldFromC0", 0,
                            path) != 0;
  g_config.barnyardUseGameSetTransformsForViewProjection =
      GetPrivateProfileIntA("CameraProxy",
                            "BarnyardUseGameSetTransformsForViewProjection", 1,
                            path) != 0;
  g_config.disableGameInputWhileMenuOpen =
      GetPrivateProfileIntA("CameraProxy", "DisableGameInputWhileMenuOpen", 0,
                            path) != 0;
  g_config.setTransformBypassProxyWhenGameProvides =
      GetPrivateProfileIntA("CameraProxy",
  ...
  if (g_config.gameProfile[0] != '\0' &&
      g_activeGameProfile == GameProfile_None) {
    snprintf(g_profileStatusMessage, sizeof(g_profileStatusMessage),
             "Unknown GameProfile='%s'. Falling back to structural detection.",
             g_config.gameProfile);
  }
  g_config.imguiScale =
      GetPrivateProfileIntA("CameraProxy", "ImGuiScalePercent", 100, path) /
      100.0F;
  if (g_config.imguiScale < 0.5F) {
    g_config.imguiScale = 0.5F;
}
  if (g_config.imguiScale > 3.0F) {
    g_config.imguiScale = 3.0F;
}
  GetPrivateProfileStringA("CameraProxy", "RemixDllName", "d3d9_remix.dll",
                           g_config.remixDllName, MAX_PATH, path);
  g_probeTransposedLayouts =
      GetPrivateProfileIntA("CameraProxy", "ProbeTransposedLayouts", 1, path) !=
      0;
  g_probeInverseView =
      GetPrivateProfileIntA("CameraProxy", "ProbeInverseView", 1, path) != 0;
  g_overrideScopeMode = GetPrivateProfileIntA(
      "CameraProxy", "OverrideScopeMode", Override_Sticky, path);
  g_overrideNFrames =
      GetPrivateProfileIntA("CameraProxy", "OverrideNFrames", 3, path);
  g_config.hotkeyToggleMenuVk =
      GetPrivateProfileIntA("CameraProxy", "HotkeyToggleMenuVK", VK_F10, path);
  g_config.hotkeyTogglePauseVk =
      GetPrivateProfileIntA("CameraProxy", "HotkeyTogglePauseVK", VK_F9, path);
  g_config.hotkeyEmitMatricesVk =
      GetPrivateProfileIntA("CameraProxy", "HotkeyEmitMatricesVK", VK_F8, path);
  g_config.hotkeyResetMatrixOverridesVk = GetPrivateProfileIntA(
      "CameraProxy", "HotkeyResetMatrixOverridesVK", VK_F7, path);
  g_config.enableCombinedDecomposition =
      GetPrivateProfileIntA("CameraProxy", "EnableCombinedDecomposition", 1,
                            path) != 0;
  g_config.combinedDecompositionLog =
      GetPrivateProfileIntA("CameraProxy", "CombinedDecompositionLog", 0,
                            path) != 0;
  g_config.allowGeneratedProjectionForVPDecomposition =
      GetPrivateProfileIntA("CameraProxy",
                            "AllowGeneratedProjectionForVPDecomposition", 0,
                            path) != 0;

  g_config.experimentalCustomProjectionEnabled =
      GetPrivateProfileIntA(
          "CameraProxy", "ExperimentalCustomProjectionEnabled", 0, path) != 0;
  int rawMode = GetPrivateProfileIntA(
      "CameraProxy", "ExperimentalCustomProjectionMode", 2, path);
  g_config.experimentalCustomProjectionMode =
      (rawMode == CustomProjectionMode_Manual) ? CustomProjectionMode_Manual
                                               : CustomProjectionMode_Auto;
  g_config.experimentalCustomProjectionOverrideDetectedProjection =
      GetPrivateProfileIntA(
          "CameraProxy",
  ...
  g_config.experimentalCustomProjectionOverrideCombinedMVP =
      GetPrivateProfileIntA("CameraProxy",
                            ...
                            0, path) != 0;
  g_config.experimentalInverseViewAsWorld =
      GetPrivateProfileIntA("CameraProxy", "ExperimentalInverseViewAsWorld", 0,
                            path) != 0;
  g_config.experimentalInverseViewAsWorldAllowUnverified =
      GetPrivateProfileIntA("CameraProxy",
                            "ExperimentalInverseViewAsWorldAllowUnverified", 0,
                            path) != 0;
  g_config.experimentalInverseViewAsWorldFast =
      GetPrivateProfileIntA("CameraProxy", "ExperimentalInverseViewAsWorldFast",
                            0, path) != 0;
  g_config.mgrrUseAutoProjectionWhenC4Invalid =
      GetPrivateProfileIntA("CameraProxy", "MGRRUseAutoProjectionWhenC4Invalid",
                            0, path) != 0;

  char customBuf[64] = {};
  GetPrivateProfileStringA("CameraProxy",
                           "ExperimentalCustomProjectionAutoFovDeg", "60.0",
                           customBuf, sizeof(customBuf), path);
  ...
  GetPrivateProfileStringA("CameraProxy",
                           "ExperimentalCustomProjectionAutoAspectFallback",
                           "1.7777778", customBuf, sizeof(customBuf), path);
  g_config.experimentalCustomProjectionAutoAspectFallback =
      (float)atof(customBuf);
  int rawHandedness = GetPrivateProfileIntA(
      "CameraProxy", "ExperimentalCustomProjectionAutoHandedness",
      ProjectionHandedness_Left, path);
  g_config.experimentalCustomProjectionAutoHandedness =
      (rawHandedness == ProjectionHandedness_Right) ? ProjectionHandedness_Right
                                                    : ProjectionHandedness_Left;

  D3DMATRIX defaultManualProjection = {};
  CreateProjectionMatrixWithHandedness(
      &defaultManualProjection, 60.0F * (3.14159265F / 180.0F), 16.0F / 9.0F,
      0.1F, 1000.0F, ProjectionHandedness_Left);
  g_config.experimentalCustomProjectionManualMatrix = defaultManualProjection;
  auto *manualValues = reinterpret_cast<float *>(
      &g_config.experimentalCustomProjectionManualMatrix);
  const auto *defaultManualValues =
      reinterpret_cast<const float *>(&defaultManualProjection);
  for (int i = 0; i < 16; ++i) {
    int row = (i / 4) + 1;
    int col = (i % 4) + 1;
    char key[64] = {};
    ...
    GetPrivateProfileStringA("CameraProxy", key, customBuf, customBuf,
                             sizeof(customBuf), path);
    manualValues[i] = (float)atof(customBuf);
  }

  char buf[64];
  GetPrivateProfileStringA("CameraProxy", "MinFOV", "0.1", buf, sizeof(buf),
                           path);
  g_config.minFOV = (float)atof(buf);
  GetPrivateProfileStringA("CameraProxy", "MaxFOV", "2.5", buf, sizeof(buf),
                           path);
...
static void EnsureProxyInitialized() {
  std::call_once(g_initOnce, []() {
    ...
    if (g_config.enableLogging) {
      ...
      LogMsg("Game executable path: %s",
             g_gameExePath[0] != 0 ? g_gameExePath : "<unknown>");
      LogMsg("Game executable name: %s",
             g_gameExeName[0] != 0 ? g_gameExeName : "<unknown>");
      LogMsg("Game display name: %s",
             g_gameDisplayName[0] != 0 ? g_gameDisplayName : "<unknown>");
    }
    g_hD3D9 = LoadTargetD3D9();
    if (g_hD3D9 != nullptr) {
      g_origDirect3DCreate9 =
          (Direct3DCreate9_t)GetProcAddress(g_hD3D9, "Direct3DCreate9");
      g_origDirect3DCreate9Ex =
          (Direct3DCreate9Ex_t)GetProcAddress(g_hD3D9, "Direct3DCreate9Ex");
      g_origD3DPERF_BeginEvent =
          (D3DPERF_BeginEvent_t)GetProcAddress(g_hD3D9, "D3DPERF_BeginEvent");
      g_origD3DPERF_EndEvent =
          (D3DPERF_EndEvent_t)GetProcAddress(g_hD3D9, "D3DPERF_EndEvent");
      g_origD3DPERF_GetStatus =
          (D3DPERF_GetStatus_t)GetProcAddress(g_hD3D9, "D3DPERF_GetStatus");
      g_origD3DPERF_QueryRepeatFrame =
          (D3DPERF_QueryRepeatFrame_t)GetProcAddress(
              g_hD3D9, "D3DPERF_QueryRepeatFrame");
      g_origD3DPERF_SetMarker =
          (D3DPERF_SetMarker_t)GetProcAddress(g_hD3D9, "D3DPERF_SetMarker");
      g_origD3DPERF_SetOptions =
          (D3DPERF_SetOptions_t)GetProcAddress(g_hD3D9, "D3DPERF_SetOptions");
      g_origD3DPERF_SetRegion =
          (D3DPERF_SetRegion_t)GetProcAddress(g_hD3D9, "D3DPERF_SetRegion");
    }
  });
}
...
static void CopyFilenameFromPath(const char *fullPath, char *outName,
  ...
  const char *filename = lastSlash != nullptr ? (lastSlash + 1) : fullPath;
  snprintf(outName, outNameSize, "%s", filename);
}

static auto TryReadFileDescription(const char *exePath, char *outDescription,
                                   size_t outDescriptionSize) -> bool {
  if ((exePath == nullptr) || (exePath[0] == 0) || (outDescription == nullptr) || outDescriptionSize == 0) {
    return false;
}
  outDescription[0] = '\0';
  ...
  if (GetFileVersionInfoA(exePath, 0, infoSize, infoData.data()) == 0) {
    return false;
}
  ...
  if ((VerQueryValueA(infoData.data(), "\\VarFileInfo\\Translation",
                      reinterpret_cast<void **>(&translate), &translateSize) == 0) ||
      (translate == nullptr) || translateSize < sizeof(LangAndCodepage)) {
    return false;
  }

  char queryPath[64] = {};
  snprintf(queryPath, sizeof(queryPath),
           R"(\StringFileInfo\%04x%04x\FileDescription)", translate[0].language,
           translate[0].codePage);

  char *value = nullptr;
  UINT valueSize = 0;
  if ((VerQueryValueA(infoData.data(), queryPath,
                      reinterpret_cast<void **>(&value), &valueSize) == 0) ||
...
IDirect3D9 *WINAPI Proxy_Direct3DCreate9(UINT SDKVersion) {
  ...
  if (g_origDirect3DCreate9 == nullptr) {
...
HRESULT WINAPI Proxy_Direct3DCreate9Ex(UINT SDKVersion, IDirect3D9Ex **ppD3D) {
  ...
  if (g_origDirect3DCreate9Ex == nullptr) {
  ...
  if (SUCCEEDED(hr) && (realD3D9Ex != nullptr)) {
...
int WINAPI Proxy_D3DPERF_BeginEvent(D3DCOLOR col, LPCWSTR name) {
  if (g_origD3DPERF_BeginEvent != nullptr) {
    return g_origD3DPERF_BeginEvent(col, name);
}
  return 0;
}

int WINAPI Proxy_D3DPERF_EndEvent(void) {
  if (g_origD3DPERF_EndEvent != nullptr) {
    return g_origD3DPERF_EndEvent();
}
  return 0;
}

DWORD WINAPI Proxy_D3DPERF_GetStatus(void) {
  if (g_origD3DPERF_GetStatus != nullptr) {
    return g_origD3DPERF_GetStatus();
}
  return 0;
}

BOOL WINAPI Proxy_D3DPERF_QueryRepeatFrame(void) {
  if (g_origD3DPERF_QueryRepeatFrame != nullptr) {
    return g_origD3DPERF_QueryRepeatFrame();
}
  return FALSE;
}

void WINAPI Proxy_D3DPERF_SetMarker(D3DCOLOR col, LPCWSTR name) {
  if (g_origD3DPERF_SetMarker != nullptr) {
    g_origD3DPERF_SetMarker(col, name);
}
}

void WINAPI Proxy_D3DPERF_SetOptions(DWORD options) {
  if (g_origD3DPERF_SetOptions != nullptr) {
    g_origD3DPERF_SetOptions(options);
}
}

void WINAPI Proxy_D3DPERF_SetRegion(D3DCOLOR col, LPCWSTR name) {
  if (g_origD3DPERF_SetRegion != nullptr) {
    g_origD3DPERF_SetRegion(col, name);
}
}
}
